 ...
 * so tool-studio's visualization list looks like the depictio Catalog result
...
export function bindsOf(render: RenderSpec): [string, string][] {
  switch (render.component) {
    ...
    case 'advanced_viz':
      // A list-typed role (a sankey's steps) shows as one badge listing its
      // columns in order, which is what the binding means.
      return Object.entries(render.roles).map(([role, value]) => [
        role,
