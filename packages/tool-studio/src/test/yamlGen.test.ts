...
describe('renderToFlow', () => {
  ...
  it('serializes a list-typed role as a flow sequence', () => {
    const r: RenderSpec = { uid: 'r5', component: 'advanced_viz', kind: 'sankey', roles: { steps: ['sample', 'lineage'] } };
    expect(renderToFlow(r)).toBe('{ component: advanced_viz, kind: sankey, roles: {steps: [sample, lineage]} }');
  });
