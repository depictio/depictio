/**
 * "Sign in with GitHub" for the static Tool Studio — a popup OAuth flow that
...
export function devToken(): string | null {
  ...
  if (raw.startsWith('$(') || raw.includes('gh auth token')) {
    ...
    console.warn('[tool-studio] VITE_GH_TOKEN looks like a literal shell command. Put the actual token value.');
    return null;
  }
  return raw;
}
