...
export async function openCatalogPr(
  ...
  const body = [
    '## Summary',
    `Adds the **${entry.toolId}** tool to the catalog, authored with [Depictio Tool Studio](https://depictio.github.io/depictio-tool-studio/).`,
    '',
    `## Files (\`${dir}/\`)`,
    '| File | Purpose |',
    '| --- | --- |',
    ...
    `| \`${entry.fixtureName}\` | Fixture that grounds the bindings in CI (nothing computed server-side). |`,
    '',
    '## Validation',
...
export async function openNewOutputPr(
  ...
  const body = [
    ...
    `Adds a new output **${entry.outputSlug}** to the existing **${entry.toolId}** tool, ` +
      'authored with [Depictio Tool Studio](https://depictio.github.io/depictio-tool-studio/).',
    '',
    `## Files (\`${dir}/\`)`,
    '| File | Purpose |',
    '| --- | --- |',
      ...
      resolveFiles: async ({ readFile }) => {
        ...
        const body = [
          ...
          `Adds ${count} visualization${plural} to the existing **${args.toolId}** tool ` +
            `(output \`${args.outputSlug}\`), authored with [Depictio Tool Studio](https://depictio.github.io/depictio-tool-studio/).`,
          '',
