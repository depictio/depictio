/**
 * "Add visualization" flow in a modal: depictio's component-type grid → the
 * per-type design surface (depictio controls + client preview). On confirm the
 * builder store (or the advanced-viz draft) is translated to a catalog
 * RenderSpec and handed back to the host.
 */
...
import DesignArea, { type AdvancedDraft } from './DesignArea';
...
export default function AddComponentModal({
  ...
  const [advanced, setAdvanced] = useState<AdvancedDraft>({ kind: '', roles: {} });
  const [seeding, setSeeding] = useState(false);

  const reset = () => {
    setType(null);
    setAdvanced({ kind: '', roles: {} });
  };
  ...
  const confirm = () => {
    let spec: RenderSpec | null = null;
    if (type === 'advanced_viz') {
      if (!advanced.kind) {
        notifications.show({ color: 'red', message: 'Pick an advanced visualization.' });
        return;
      }
      const desc = kinds[advanced.kind];
      const missing = (desc?.required_roles ?? []).filter((r) => !advanced.roles[r]);
      if (missing.length) {
        notifications.show({ color: 'red', message: `Bind required role(s): ${missing.join(', ')}.` });
        return;
      }
      spec = { uid: nextUid(), component: 'advanced_viz', kind: advanced.kind, roles: advanced.roles };
    } else {
      spec = renderSpecFromStore();
    }
    if (!spec) {
    ...
    onAdd(spec);
    notifications.show({ color: 'teal', message: 'Visualization added.' });
    close();
  };
      ...
      {!type ? (
        ...
        <>
          ...
          <Group justify="space-between" pt="sm" style={{ flexShrink: 0 }}>
            ...
            <Button color="green" leftSection={<Icon icon="mdi:plus" />} onClick={confirm}>
              Add to output
            </Button>
          </Group>
        </>
