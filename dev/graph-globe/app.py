...
for j, city_dict in enumerate(locations):
    random.seed(j)
    locations_graph[city_dict["name"]] = Graph(random.randint(17,22), city_dict["name"]) 
...
leaflet_map = dl.Map(
    ...
    id="leaflet-map",
    style={"width": "80%", "height": "50vh", "margin": "auto", "display": "block"},
)
...
app.layout = html.Div(
    [
        ...
        html.H5(id="node-info", children="No node selected"),
        modal,
    ]
)
...
@app.callback(
    Output("markers", "style"),
    [Input("markers", "click_feature")],
)
def marker_click(feature):
    if feature is not None:
        # Define your highlight style, this could be changing the color, increasing the size, etc.
        highlight_style = {"color": "yellow", "weight": 10}

        # Get the feature's properties
        feature_props = feature.get('properties', {})

        # Set the highlight style on the clicked feature
        feature_props['style'] = highlight_style

        return feature_props['style']
    return None
...
def update_info(feature, node_data, elements):
    ...
    if not ctx.triggered:
        return elements, "No node selected"
    else:
    ...
    if input_id == "markers":
        if feature is not None:
            # Generate new graph here based on the clicked location
            ...
            # # Use lat and lon as seeds for random
            # lat, lon = feature["geometry"]["coordinates"]
            # random.seed(lat)
            # np.random.seed(int(lon))
            # # Create nodes
            # nodes = []
            # num_nodes = random.randint(17, 20)

            # # Create edges
            # edges = []

            # nodes, edges = build_graph(num_nodes, nodes, edges)

            return nodes + edges, f"You clicked on {name} marker"
        else:
            return elements, "No marker selected"
