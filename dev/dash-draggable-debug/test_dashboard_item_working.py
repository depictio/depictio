...
def create_working_test():
    ...
    app.layout = html.Div([
        ...
        html.Div([
            ...
            html.Div(id="layout-info", children="Move or resize components to see layout updates..."),
        ]),
        html.Hr(),
    ...
    def show_layout_info(layouts):
        ...
        for breakpoint, items in layouts.items():
            ...
            for i, item in enumerate(items):
                uuid_status = "✅ UUID preserved" if item.get('i', '').startswith('box-') else "❌ UUID lost"
                info.append(html.P([
                    f"Item {i}: ",
                    html.Code(f"id={item.get('i', 'unknown')}"),
                    f" at ({item.get('x', 0)}, {item.get('y', 0)}) size {item.get('w', 0)}x{item.get('h', 0)} ",
                    html.Span(uuid_status, style={"color": "green" if uuid_status.startswith("✅") else "red"})
                ]))
