...
def create_browser_storage_demo():
    ...
    @app.callback(
        [Output("storage-display", "children"), Output("session-storage", "data")],
                ...
                for i, item in enumerate(layouts[breakpoint]):
                    display_components.append(
                        html.Div(
                            [
                                html.Code(
                                    f"Item {i}: id={item.get('i', 'unknown')} at ({item.get('x', 0)}, {item.get('y', 0)}) size {item.get('w', 0)}x{item.get('h', 0)}"
                                )
                            ],
                ...
                for i, item in enumerate(uuid_layouts[breakpoint]):
                    ...
                    display_components.append(
                        html.Div(
                            [
                                html.Code(
                                    f"Item {i}: id={short_uuid}... at ({item.get('x', 0)}, {item.get('y', 0)}) size {item.get('w', 0)}x{item.get('h', 0)}"
                                )
                            ],
