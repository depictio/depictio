"""Synthetic DE results data for the progressive filter prototype."""
...
def generate_de_data(n_genes: int = 2000, seed: int = 42) -> pd.DataFrame:
    """Generate synthetic differential expression results.

    Creates a dataset with a mixture of non-significant genes (centered around 0)
    and significantly up/down-regulated genes, mimicking a typical DE analysis output.
    ...
    Returns:
        DataFrame with columns: gene_name, log2fc, pvalue, padj, neg_log10_pvalue,
    ...
    # Adjusted p-values (BH-like: slightly larger than raw p-values)
    padj = np.clip(pvalue * rng.uniform(1.0, 3.0, n_genes), 0, 1)
    ...
    # Significance classification (|log2FC| > 1.5 AND padj < 0.05)
