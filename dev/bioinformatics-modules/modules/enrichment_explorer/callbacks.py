"""Callbacks for the Enrichment Explorer module.

Handles contrast/source filtering, enrichment table updates, running ES plot,
dot plot summary, leading edge heatmap, and cross-module communication via
shared stores.

All component IDs are prefixed with ``ee-``.
"""
...
def _compute_running_es(
    ...
    positions : np.ndarray
        Array of rank positions (x-axis).
    running_es : np.ndarray
        Array of running ES values (y-axis).
    hit_indices : list[int]
        Indices where the gene is in the set (for barcode).
    """
...
def register_callbacks(app: Dash, data: dict) -> None:
    """Register all Enrichment Explorer callbacks on the given app.
    ...
    """
    enrichment_df = data["enrichment_df"]
    ranked_genes = data["ranked_genes"]
    expression_df = data["expression_df"]
    metadata_df = data["metadata_df"]

    # ── Sync contrast from shared store ──────────────────────
    @app.callback(
        Output("ee-contrast-select", "value"),
        Input(ACTIVE_CONTRAST, "data"),
    )
    def sync_contrast_from_store(active_contrast: str | None) -> str | None:
        if not active_contrast:
            return no_update
        # Only update if the contrast exists in our enrichment data
        available = enrichment_df["contrast"].unique().tolist()
        if active_contrast in available:
            return active_contrast
        return no_update

    # ── Update summary badges ────────────────────────────────
    @app.callback(
        Output("ee-badge-sig-pathways", "children"),
        Output("ee-badge-active-contrast", "children"),
        Output("ee-badge-top-nes", "children"),
        Input("ee-contrast-select", "value"),
        Input("ee-source-filter", "value"),
    )
    def update_badges(contrast: str | None, sources: list[str] | None):
        if not contrast or not sources:
            return "0 significant", "No contrast", "Top NES: 0.00"

        mask = (enrichment_df["contrast"] == contrast) & (enrichment_df["source"].isin(sources))
        filtered = enrichment_df.loc[mask]
        sig_count = int((filtered["padj"] < 0.05).sum())
        if len(filtered) > 0:
            top_nes_val = filtered.loc[filtered["NES"].abs().idxmax(), "NES"]
        else:
            top_nes_val = 0.0
        ...
        return (
            ...
            contrast.replace("_", " "),
            f"Top NES: {top_nes_val:.2f}",
        )

    # ── 1. Filter enrichment table ───────────────────────────
    @app.callback(
        Output("ee-enrichment-table", "rowData"),
        Input("ee-contrast-select", "value"),
        Input("ee-source-filter", "value"),
    )
    ...
    @app.callback(
        Output("ee-running-es-plot", "figure"),
        Input("ee-enrichment-table", "selectedRows"),
        Input("ee-contrast-select", "value"),
    )
    ...
    # ── 3. Dot plot summary ──────────────────────────────────
    @app.callback(
        Output("ee-dot-plot", "figure"),
        Input("ee-contrast-select", "value"),
        Input("ee-source-filter", "value"),
        Input("ee-top-n-slider", "value"),
    )
    ...
    @app.callback(
        Output("ee-le-heatmap", "figure"),
        Input("ee-enrichment-table", "selectedRows"),
        Input("ee-contrast-select", "value"),
        Input(FILTERED_FEATURE_IDS, "data"),
        Input(ACTIVE_FEATURE, "data"),
    )
    def update_le_heatmap(
        selected_rows,
        contrast: str,
        filtered_ids: list[str] | None,
        ...
        cond_bar = [cond_colors.get(c, "#999") for c in metadata_df["condition"]]
        ...
        # Highlight filtered genes in leading edge
        shapes = [
            dict(
                type="rect",
                x0=i - 0.5,
        ...
        # If filtered feature IDs are provided, highlight matching genes on the y-axis
        if filtered_ids:
            filtered_set = set(filtered_ids)
            for gene_idx, gene in enumerate(available):
