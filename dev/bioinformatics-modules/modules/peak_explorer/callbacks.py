"""Callbacks for the Peak Explorer module.
...
3. Cross-module: writes filtered gene names to ``filtered-feature-ids-store``,
   reads ``highlighted-samples-store`` (no-op for peaks).

All component IDs are prefixed with ``pe-`` to avoid conflicts.
"""
...
def register_callbacks(app: Dash, data: dict) -> None:
    ...
    # ── Add filter callback ──────────────────────────────────────────────────
    @app.callback(
        Output("pe-filter-store", "data", allow_duplicate=True),
        Output("pe-next-filter-id", "data", allow_duplicate=True),
        Output("pe-filter-cards-container", "children", allow_duplicate=True),
        Input("pe-add-filter-btn", "n_clicks"),
        State("pe-filter-store", "data"),
        State("pe-next-filter-id", "data"),
        State("pe-filter-cards-container", "children"),
    ...
    @app.callback(
        ...
        Output("pe-funnel-chart", "figure"),
        Output("pe-enrichment-curve", "figure"),
        Output("pe-annotation-pie", "figure"),
        Output("pe-summary-badges", "children"),
        Output("pe-peak-table", "rowData"),
        Output("pe-peak-table", "columnDefs"),
        Output({"type": "pe-filter-count-badge", "index": ALL}, "children"),
        Output({"type": "pe-cat-controls", "index": ALL}, "style"),
        Output({"type": "pe-numeric-controls", "index": ALL}, "style"),
        Output({"type": "pe-threshold-container", "index": ALL}, "style"),
        Output({"type": "pe-filter-cat-values", "index": ALL}, "data"),
        Output(FILTERED_FEATURE_IDS, "data", allow_duplicate=True),
        Input({"type": "pe-filter-enabled", "index": ALL}, "checked"),
        Input({"type": "pe-filter-column", "index": ALL}, "value"),
        Input({"type": "pe-filter-operator", "index": ALL}, "value"),
        Input({"type": "pe-filter-threshold", "index": ALL}, "value"),
        Input({"type": "pe-filter-abs", "index": ALL}, "checked"),
        Input({"type": "pe-filter-cat-values", "index": ALL}, "value"),
    ...
    def update_main(
        ...
        # Cross-module: write filtered gene names to shared store
        filtered_gene_names = filtered_df["nearest_gene"].unique().tolist()
