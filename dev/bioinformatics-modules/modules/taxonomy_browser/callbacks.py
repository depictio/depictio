"""Callbacks for the Taxonomy Browser module.
...
- READS from ``highlighted-samples-store``: highlights specific samples in
  stacked bar and diversity plots.
- WRITES to ``selected-features-store``: selected taxon names.

All component IDs are prefixed with ``tb-`` to avoid conflicts.
"""
...
from shared_stores import HIGHLIGHTED_SAMPLES, SELECTED_FEATURES
...
def _compute_alpha_diversity(counts_df: pd.DataFrame) -> pd.DataFrame:
...
def _build_stacked_bar(
    ...
    conditions: list[str],
    abundance_threshold: float,
    highlighted_samples: list[str] | None = None,
) -> go.Figure:
    ...
    Highlighted samples get a border annotation if provided.
    """
    ...
    # Highlight samples if provided
    if highlighted_samples:
        for sample_id in highlighted_samples:
            if sample_id in plot_df.columns:
                col_idx = plot_df.columns.tolist().index(sample_id)
                fig.add_vline(
                    x=col_idx,
                    line_dash="dot",
                    line_color="#e64980",
...
def _build_alpha_diversity(
    ...
    Highlighted samples are shown with distinct markers if provided.
    """
    diversity = _compute_alpha_diversity(counts_df)
    ...
    if highlighted_samples:
        hl_sub = merged[merged["sample"].isin(highlighted_samples)]
        if not hl_sub.empty:
            fig.add_trace(
                go.Scatter(
                    x=hl_sub["condition"],
                    ...
                    mode="markers",
                    marker=dict(
                        color="#e64980", size=12, symbol="star", line=dict(width=1, color="#333")
                    ),
                    name="Highlighted",
                    text=hl_sub["sample"],
                    hovertemplate="<b>%{text}</b><br>Shannon: %{y:.3f}<extra></extra>",
                )
            )
...
def register_callbacks(app: Dash, data: dict) -> None:
    ...
    """
    abundance_df = data["taxonomy_abundance_df"]
    taxonomy_df = data["taxonomy_df"]
    metadata_df = data["taxonomy_metadata_df"]

    @app.callback(
        Output("tb-summary-badges", "children"),
        Output("tb-differential-scatter", "figure"),
        Output("tb-stacked-bar", "figure"),
        Output("tb-alpha-diversity-chart", "figure"),
        Output("tb-abundance-table", "rowData"),
        Output("tb-abundance-table", "columnDefs"),
        Output("tb-sidebar-summary", "children"),
        Output(SELECTED_FEATURES, "data", allow_duplicate=True),
        Input("tb-rank-selector", "value"),
        Input("tb-abundance-threshold-slider", "value"),
        Input("tb-top-n-input", "value"),
        Input("tb-condition-filter", "value"),
    ...
    def update_all(
        ...
        agg_df = _aggregate_by_rank(abundance_df, taxonomy_df, rank)
        ...
        bar_fig = _build_stacked_bar(
            ...
            conditions,
            abundance_threshold,
            highlighted_samples=highlighted_samples,
        )

        # -- Alpha diversity (uses raw counts, not filtered) --
        alpha_fig = _build_alpha_diversity(
            abundance_df,
            metadata_df,
            conditions,
            highlighted_samples=highlighted_samples,
        ...
        # Cross-module: write selected taxon names (those above threshold)
        selected_taxa = list(taxa_above)
