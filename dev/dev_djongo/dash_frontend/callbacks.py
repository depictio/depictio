...
def register_callbacks(app):
    ...
    @app.callback(Output("auth-modals", "children"), Input("url", "pathname"))
    def render_auth_modals(pathname):
        return auth_modals()

    # Callback to render header based on authentication state
    @app.callback(Output("header", "children"), Input("auth-store", "data"))
    ...
    def toggle_login_modal(
        hidden_trigger_clicks, close_clicks, switch_clicks, login_clicks, is_open
    ):
        ctx = callback_context
    ...
    def toggle_register_modal(
        switch_clicks,
        close_clicks,
        switch_to_login_clicks,
        register_clicks,
        hidden_trigger_clicks,
        is_open,
    ):
        ctx = callback_context
        if not ctx.triggered:
    ...
    def login_user(n_clicks, username, password):
        ...
        if status_code == 200:
            ...
            # Failed login
            error_msg = (
                "Invalid username or password" if status_code == 401 else f"Error: {response}"
            )
