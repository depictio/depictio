...
def create_markdown_input(component_id, placeholder="# Section Title", initial_value="# Dashboard Section"):
...
def render_markdown_to_dash(text):
    ...
    for line in lines:
        ...
        if line.startswith('##### '):
            ...
            dash_components.append(
                dmc.Text(line, c="gray", style={"margin": "4px 0"})
            )
    
    return dash_components if dash_components else [dmc.Text("Start typing...", c="dimmed", size="sm")]

# Test layout with multiple markdown inputs
app.layout = dmc.MantineProvider([
    dmc.Container([
        dmc.Title("Simple Markdown Text Input Prototype", order=1),
        dmc.Text("Perfect for dashboard section delimiters with # header syntax", c="dimmed"),
        dmc.Space(h="md"),
        ...
        create_markdown_input("section-1", "# Main Section Title", "# Data Analysis Dashboard"),
        ...
        dmc.Card([
            ...
            dmc.Text("This would be where additional context or documentation appears.", size="sm")
        ], withBorder=True, shadow="sm", radius="md", p="md"),
...
def handle_button_clicks(h1_clicks, h2_clicks, h3_clicks, h4_clicks, h5_clicks, clear_clicks, current_value):
