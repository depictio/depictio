...
class FlaskSecurityScanner:
    ...
    def _check_debug_mode(self, assessment: ServerAssessment):
        ...
        for test_path in debug_test_paths:
            try:
                ...
                found_indicators = [indicator for indicator in debug_indicators if indicator in content]
    ...
    def _check_dash_configuration(self, assessment: ServerAssessment):
        ...
        try:
            ...
            config_match = re.search(r'<script id="_dash-config" type="application/json">(.*?)</script>', 
    ...
    def _print_summary_report(self, assessments: List[ServerAssessment]):
        ...
        total_critical = sum(len([f for f in a.findings if f.severity == "CRITICAL"]) for a in assessments)
        total_high = sum(len([f for f in a.findings if f.severity == "HIGH"]) for a in assessments)
        total_medium = sum(len([f for f in a.findings if f.severity == "MEDIUM"]) for a in assessments)
