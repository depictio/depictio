...
| `/report` | a portal-shaped page: five in-page tabs, a collapsible five-control deck, and components drawn from **two** dashboards |
...
`/report` is the widest of them. Its tab strip is in-page rather than five
routes, so the filter deck above it keeps its state as the reader moves between
tabs; its Overview tab draws MultiQC panels from the *ampliseq* dashboard while
every other tab draws from *community*, which is the demonstration that a host
page is not restricted to one dashboard's manifest. Every component on it, down
to the control deck itself, carries the same footer: a badge saying whether it
arrived as a Plotly spec or as a frame, and a disclosure holding the request,
its parameters and the response's provenance. Note that `shoot_all.py` captures
only the tab a page opens on.
