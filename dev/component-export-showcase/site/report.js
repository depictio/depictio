...
/* Every control on this page belongs to the same dashboard as every figure, so
 * every filter is in scope for every figure.
 *
 * It used to require entry.dc === dc, which silently disabled two of the three
 * controls: `habitat` and `sampling_date` live on the metadata collection, and
 * no figure here reads that collection — quant is taxonomy, stacked/sunburst/
 * sankey are their own canonicals. Only `Kingdom` ever reached the API.
 *
 * Dropping them client-side was the bug: the project links metadata to those
 * collections, so Depictio resolves the join server-side and the filter does
 * mean something. Same rule /qc-report uses (example_clean.js): a filter
 * applies when the dashboard matches OR the collection does. Here the dashboard
 * always matches, so the collection never has to. */
