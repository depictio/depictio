...
def test_security_features():
    ...
    test_cases = [
        ...
        ("Valid pandas operations", "df_new = df.groupby('category').mean(); fig = px.bar(df_new, x=df_new.index, y='x')", True),
        ("Valid numpy operations", "import numpy as np; fig = px.scatter(df, x='x', y=np.log(df['y']))", True),
        ...
        ("File operations", "with open('test.txt', 'w') as f: f.write('test'); fig = px.scatter(df, x='x', y='y')", False),
    ...
    for description, code, should_pass in test_cases:
        ...
        if actual_result == expected_result:
            ...
            print(f"❌ FAIL - Expected: {'Pass' if expected_result else 'Fail'}, Got: {'Pass' if actual_result else 'Fail'}")
            if not success:
...
def test_allowed_operations():
    ...
    allowed_operations = [
        ...
        ("Pandas groupby", "df_agg = df.groupby('category').mean(); fig = px.bar(df_agg, x=df_agg.index, y='x')"),
        ("Numpy operations", "import numpy as np; fig = px.scatter(df, x='x', y=np.sqrt(df['y']))"),
        ("Custom styling", "fig = px.scatter(df, x='x', y='y'); fig.update_layout(title='Test Plot')"),
        ("Multiple traces", "import plotly.graph_objects as go; fig = go.Figure(); fig.add_trace(go.Scatter(x=df['x'], y=df['y'], mode='markers'))"),
        ("Date operations", "from datetime import datetime; fig = px.scatter(df, x='x', y='y', title=str(datetime.now()))"),
    ]
