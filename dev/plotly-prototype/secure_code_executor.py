...
class SecureCodeValidator:
    ...
    ALLOWED_MODULES = {
        'pandas': {
            'pd': ['DataFrame', 'Series', 'concat', 'merge', 'pivot_table', 'crosstab', 
                   'cut', 'qcut', 'to_datetime', 'to_numeric', 'get_dummies', 'melt',
                   'groupby', 'agg', 'transform', 'apply', 'map', 'replace', 'fillna',
                   'dropna', 'drop_duplicates', 'sort_values', 'reset_index', 'set_index']
        },
        'plotly.express': {
            'px': ['scatter', 'line', 'bar', 'histogram', 'box', 'violin', 'pie', 'area',
                   'density_heatmap', 'density_contour', 'scatter_3d', 'line_3d', 'bar_polar',
                   'line_polar', 'scatter_polar', 'choropleth', 'scatter_mapbox', 'line_mapbox',
                   'density_mapbox', 'choropleth_mapbox', 'sunburst', 'treemap', 'icicle',
                   'funnel', 'funnel_area', 'strip', 'ecdf', 'scatter_matrix', 'parallel_coordinates',
                   'parallel_categories', 'imshow', 'timeline', 'colors']
        },
        'plotly.graph_objects': {
            'go': ['Figure', 'Scatter', 'Bar', 'Line', 'Histogram', 'Box', 'Violin', 'Pie',
                   'Heatmap', 'Contour', 'Surface', 'Mesh3d', 'Scatter3d', 'Layout', 'Annotation',
                   'Scatterpolar', 'Barpolar', 'Scattermapbox', 'Choroplethmapbox', 'Densitymapbox',
                   'Sunburst', 'Treemap', 'Icicle', 'Funnel', 'Funnelarea', 'Indicator', 'Table']
        },
        'numpy': {
            'np': ['array', 'arange', 'linspace', 'zeros', 'ones', 'empty', 'full', 'eye',
                   'random', 'mean', 'std', 'var', 'min', 'max', 'sum', 'prod', 'median',
                   'percentile', 'quantile', 'sort', 'argsort', 'argmin', 'argmax', 'unique',
                   'where', 'select', 'concatenate', 'stack', 'hstack', 'vstack', 'split',
                   'reshape', 'transpose', 'flatten', 'squeeze', 'expand_dims', 'sin', 'cos',
                   'tan', 'exp', 'log', 'log10', 'sqrt', 'power', 'absolute', 'sign', 'round',
                   'floor', 'ceil', 'pi', 'e', 'inf', 'nan', 'isnan', 'isinf', 'isfinite']
        },
        'datetime': {
            'datetime': ['datetime', 'date', 'time', 'timedelta', 'timezone']
        },
        'math': {
            'math': ['sqrt', 'pow', 'exp', 'log', 'log10', 'sin', 'cos', 'tan', 'asin', 'acos',
                     'atan', 'atan2', 'sinh', 'cosh', 'tanh', 'degrees', 'radians', 'pi', 'e',
                     'inf', 'nan', 'isnan', 'isinf', 'isfinite', 'floor', 'ceil', 'trunc',
                     'fabs', 'factorial', 'gcd', 'lcm', 'modf', 'frexp', 'ldexp']
        }
    }
        ...
        for node in ast.walk(tree):
            ...
            # Check function calls
                ...
                if isinstance(node.func, ast.Name):
                    dangerous_funcs = ['eval', 'exec', 'compile', '__import__', 'getattr', 'setattr', 'delattr']
            ...
            # Check attribute access
                ...
                dangerous_attrs = [
                    '__globals__', '__locals__', '__builtins__', '__code__', '__closure__',
                    '__defaults__', '__dict__', '__class__', '__bases__', '__mro__',
                    '__subclasses__', 'func_globals', 'func_code', 'func_closure', 'func_defaults'
                ]
            ...
            # Check for dangerous names
                ...
                dangerous_names = ['__import__', 'eval', 'exec', 'compile', 'globals', 'locals', 'vars', 'dir']
...
class SecureCodeExecutor:
    ...
    def execute_code(self, code: str) -> Tuple[bool, Any, str]:
        ...
        try:
            ...
            if fig is None:
                return False, None, "No Plotly figure found in code. Make sure to create a figure object (e.g., fig = px.scatter(...))"
