...
Every number is on the same scale, a PER-SAMPLE mean over the samples of the
run. The catalog metric tables already carry one row per sample and are
averaged; the Cell Ranger route's CellBender call is a bare barcode list, so it
is counted per sample (distinct barcodes under each `source_path`) and then
averaged the same way. A one-sample run reads as plain counts.
...
Output schema:
    ...
    cells_called : Int64           the route's own primary cell call, per-sample mean
    cellbender_cells : Int64       CellBender's cell call for the same route, per-sample mean,
                                   null if not run
...
def _barcode_count(df: pl.DataFrame | None) -> float | None:
    """Distinct barcodes per sample, averaged: the scale `_first_row` puts the metrics on.

    The raw scan names the sample only through `source_path`; without it the
    list is taken as one sample's.
    """
    if df is None or df.height == 0 or "barcode" not in df.columns:
        return None
    sample = pl.col("source_path") if "source_path" in df.columns else pl.lit("")
    per_sample = (
        df.select(
            sample.cast(pl.Utf8).alias("source_path"),
            pl.col("barcode").cast(pl.Utf8).str.replace(_GEM_SUFFIX_RE, "").alias("barcode_core"),
        )
        .drop_nulls("barcode_core")
        .unique()
        .group_by("source_path")
        .len()
    )
    if per_sample.height == 0:
        return None
    return float(per_sample.get_column("len").mean())
