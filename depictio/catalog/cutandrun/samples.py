...
Output schema:
    ...
    replicate_label : Utf8  `R<replicate>`, the same factor as a categorical filter
...
def transform(sources: dict[str, pl.DataFrame]) -> pl.DataFrame:
    ...
    samples = samples.with_columns(
        ...
        .alias("role"),
        # The replicate is a real samplesheet factor, and a factor is filtered
        # with a MultiSelect. An Int64 column only takes a slider, so the same
        # value is carried a second time as the label the run names files with.
        pl.concat_str([pl.lit("R"), pl.col("replicate").cast(pl.Utf8)]).alias("replicate_label"),
    )
