...
def _flag(column: str, dtype: pl.DataType) -> pl.Expr:
    """A control flag as Boolean, however the samples table spells it.

    `cutandrun/samples.py` publishes a Boolean; a samplesheet bound directly
    carries `0`/`1` or `true`/`false`, and Polars does not cast Utf8 to Boolean.
    """
    if dtype == pl.Boolean:
...
def transform(sources: dict[str, pl.DataFrame]) -> pl.DataFrame:
    ...
    if "is_control" in samples.columns:
        targets = samples.filter(~_flag("is_control", samples.schema["is_control"]))
