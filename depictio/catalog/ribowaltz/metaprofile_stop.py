"""riboWaltz metagene P-site profile around the stop codon.
...
(nt)``). This recipe keeps the stop-codon window; ``metaprofile_start.py`` the
start-codon one. A clean library shows the 3-nt saw-tooth up to the stop codon
and a drop after it, where ribosomes release.

Output: one row per library and position.
    sample : Utf8       library id
    position : Int64    distance from the first nucleotide of the stop codon, nt
...
def transform(sources: dict[str, pl.DataFrame]) -> pl.DataFrame:
    """The stop-codon window, renormalised to percent per library."""
