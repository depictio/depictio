...
import { refreshManifest, useBrandAccents } from 'depictio-react-core';
...
const ManifestRefreshPanel: React.FC<ManifestRefreshPanelProps> = ({
  ...
  // Watch the run until it is terminal, the deadline passes, or the poll
  // itself keeps failing (shared with the "From a run folder" flow).
  useManifestRunPoll({
    runId,
    startedAt,
    onReport: setReport,
          ...
          <Text size="sm" data-testid="manifest-refresh-status" data-state={state}>
            ...
            {state === 'running' &&
              `Refreshing (${formatElapsed(elapsedMs)} elapsed)` +
                (report ? `: ${summarizeManifestRun(report)}` : '')}
            {state === 'success' &&
              report &&
              `Refresh completed in ${formatElapsed(elapsedMs)}: ${summarizeManifestRun(report)}`}
            {state === 'failed' &&
              (report
                ? `Refresh finished with errors in ${formatElapsed(elapsedMs)}: ${summarizeManifestRun(report)}`
                : 'Refresh failed')}
          </Text>
