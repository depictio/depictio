...
def register_callbacks_dashboards_management(app: dash.Dash) -> None:
    ...
    def create_homepage_view(dashboards, user_id, token, current_user):
        ...
        # Sort example dashboards: ampliseq first, then penguins
        example_project_order = [
            STATIC_IDS["ampliseq"]["project"],
            STATIC_IDS["penguins"]["project"],
        ]
        example_dashboards.sort(
            key=lambda d: (
                example_project_order.index(str(d.get("project_id", "")))
                if str(d.get("project_id", "")) in example_project_order
                else len(example_project_order)
            )
        )

        example_dashboards_section_header = dmc.Title(
            [
                DashIconify(icon="mdi:star", width=18, color="#fab005"),
                " Example Dashboards",
            ],
            order=3,
        )
