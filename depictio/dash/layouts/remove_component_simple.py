...
from dash import ALL, Input, Output, Patch, State, ctx
...
def register_remove_component_simple_callback(app):
    ...
    def remove_component_from_dashboard(
        ...
        if len(ctx.triggered) > 1:
            logger.warning(
                f"  ⚠️ GUARD TRIGGERED - Multiple triggers detected ({len(ctx.triggered)})"
            )
        ...
        if isinstance(current_children, list) and len(current_children) > 0:
            ...
            if isinstance(current_children[0], dict):
                logger.info(
                    f"  DEBUG: current_children[0] keys: {list(current_children[0].keys())}"
                )
        ...
        if len(current_children) == 1:
            ...
            if not grid_component:
                ...
                if len(grid_children) < 2:
                    logger.warning(
                        f"  Grid doesn't have 2 GridCol children (got {len(grid_children)})"
                    )
                    raise PreventUpdate
        ...
        if component_type == "interactive":
            ...
            if left_children:
                ...
                if idx >= 0:
                    ...
                    if has_group_wrapper:
                        ...
                        del patch[0]["props"]["children"][grid_index]["props"]["children"][0][
                            "props"
                        ]["children"]["props"]["children"][idx]
                    else:
            ...
            if right_children and len(right_children) > 0:
                ...
                if cards_grid_children:
                    logger.info(
                        f"  DEBUG: Inspecting all {len(cards_grid_children)} cards in grid..."
                    )
                    for i, child in enumerate(cards_grid_children):  # All cards
                        ...
                        if isinstance(child, dict):
                            ...
                            if (
                                isinstance(children_to_inspect, list)
                                and len(children_to_inspect) > 0
                            ):
                                first_child = children_to_inspect[0]
                                logger.info(
                                    f"  DEBUG: Card {i} first child type: {type(first_child)}"
                                )
            ...
            if not removed and right_children and len(right_children) > 1:
                ...
                if idx >= 0:
                    ...
                    logger.info(
                        f"    Removed card at index {idx} from other components using Patch()"
                    )
                    removed = True
    ...
    logger.info("✅ Patch-based remove component callback registered")
