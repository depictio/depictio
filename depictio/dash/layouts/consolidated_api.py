...
def register_consolidated_api_callbacks(app):
    ...
    logger.debug(
        "🔧 CONSOLIDATED API: Registering async callback for non-blocking HTTP requests..."
    )
    ...
    async def consolidated_user_server_and_project_data(
        ...
        if ctx.triggered:
            logger.debug(f"🔧 CONSOLIDATED CALLBACK TRIGGER: {ctx.triggered[0]['prop_id']}")
            logger.debug(
            ...
            if trigger_id != "local-store.data":
                logger.debug(f"🔧 CONSOLIDATED CALLBACK: Unexpected trigger {trigger_id}, skipping")
                return cached_user, cached_server, cached_project

        # Skip auth page
        if pathname == "/auth":
            logger.debug("🔧 CONSOLIDATED CALLBACK: Skipping auth page")
            return no_update, no_update, no_update

        # Check if we have a valid token
        if not local_store or not local_store.get("access_token"):
            logger.debug("🔧 CONSOLIDATED CALLBACK: No token found, returning None")
        ...
        if ctx.triggered and ctx.triggered[0]["prop_id"] == "local-store.data":
            ...
            if current_token == previous_token:
                logger.debug("🔄 Token unchanged, using cache-based updates only")
            else:
                logger.debug("🔄 Token changed, forcing data refresh")
                update_user = True
                update_server = True

        # If nothing needs updating, return cached data
        if not update_user and not update_server and not update_project:
            logger.debug("🔧 CONSOLIDATED CALLBACK: Using cached data, no updates needed")
            logger.debug(
        ...
        logger.debug("🔧 CONSOLIDATED CALLBACK: About to start async tasks")
        logger.debug("🚀 ASYNC MODE ENABLED: Using httpx.AsyncClient for non-blocking I/O")

        async def fetch_user_data(token):
            """Fetch user data with async HTTP client."""
            try:
                logger.debug("🔄 Consolidated API: Fetching user data (async)")
        ...
        async def fetch_server_status(token):
            ...
            try:
                logger.debug("🔄 Consolidated API: Fetching server status (async)")
        ...
        async def fetch_project_data(token, dashboard_id):
            ...
            try:
                logger.debug("🔄 Consolidated API: Fetching project data (async)")
        ...
        if update_project and dashboard_id:
            try:
                ...
                if project_result:
                    ...
                    logger.debug(f"✅ Consolidated API: Project data cached - {dashboard_id}")
            except Exception as e:
        ...
        logger.debug("🔧 CONSOLIDATED CALLBACK: No tasks executed, returning cached data")
        return cached_user, cached_server, cached_project

    logger.debug("✅ CONSOLIDATED API: Callback registered successfully!")
...
def get_cached_server_status(server_cache: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    """
    logger.debug(f"🔧 get_cached_server_status: server_cache = {server_cache}")

    if not server_cache:
        logger.debug("🔧 get_cached_server_status: No server_cache provided")
        return None
    ...
    if cache_age > 120:
        logger.debug(f"🔧 get_cached_server_status: Cache expired (age: {cache_age}s > 120s)")
        return None
    ...
    logger.debug(f"🔧 get_cached_server_status: returning {result}")
    return result
