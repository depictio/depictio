"""
Shared save utilities for card components.

This module provides reusable functions for saving card component metadata
...
def save_card_to_dashboard(
    dashboard_id: str, component_metadata: dict, token: str, app_prefix: str = "dashboard"
) -> str:
    """
    Save or update card component in dashboard.

    This function handles the complete API interaction for saving card metadata:
    ...
    Args:
        ...
        component_metadata: Complete component metadata dict with all card fields
    ...
    logger.info(f"💾 SAVE CARD - Dashboard: {dashboard_id}, Component: {component_id}")
    ...
    try:
        ...
        logger.info(f"✅ SAVE CARD SUCCESS - Component {component_id} saved")
        logger.info(f"   API Response status: {update_response.status_code}")

    except Exception as e:
        logger.error(f"❌ SAVE CARD FAILED - Error: {e}")
