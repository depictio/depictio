...
def register_callbacks_card_component(app):
    ...
    def patch_card_with_filters(filters_data, metadata, trigger_data):
        ...
        Args:
            filters_data: Interactive filter selectionsâ
        ...
        if not all([wf_id, dc_id, column_name, aggregation]):
            # Get component index from metadata for logging
            card_index = metadata.get("index", "unknown")
            logger.debug(
                f"Skipping patch for card {card_index} - no data source (dummy/random card)"
            )
