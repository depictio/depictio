...
def register_callbacks_figure_component(app):
    ...
    #     - Imports moved to module level (eliminates import overhead)
    #     - Early return check to skip unnecessary updates (biggest win)
    #     - Debug logging instead of info (reduces string formatting overhead)
    #     """
    #     # Get component index early for logging
    #     component_index = wrapper_id.get("index", "unknown") if wrapper_id else "unknown"
    #
    #     # PERFORMANCE OPTIMIZATION: Early return if no metadata
    #     if not metadata:
    #         logger.debug(f"📊 [{component_index}] Skipping popover update - no metadata")
    #         raise PreventUpdate
    #
    ...
    #
    #     # PERFORMANCE OPTIMIZATION: Skip popover update if full data already displayed
    #     # This is the BIGGEST performance win - avoids recreating complex component tree
    #     # when data counts haven't changed (e.g., during metadata updates for other reasons)
    #     if not was_sampled and displayed_count == total_count and not full_data_loaded:
    #         logger.debug(
    #             f"📊 [{component_index}] Skipping popover update - full data displayed "
    #             f"({displayed_count:,} points, no sampling)"
    #         )
    #         raise PreventUpdate
    #
    #     # Get trigger info for debugging (only if we're actually updating)
    #     ctx_obj = callback_context
    #     trigger = ctx_obj.triggered[0]["prop_id"] if ctx_obj.triggered else "initial"
    #
    #     logger.debug(
    #         f"📊 [{component_index}] Updating popover (trigger: {trigger}): "
    ...
    #     )
    #
    #     logger.debug(f"📊 [{component_index}] Returning recreated popover with fresh data")
