...
def register_callbacks_multiqc_component(app):
    ...
    def render_multiqc_plot_background(trigger_data, existing_trace_metadata):
        ...
        Args:
            trigger_data: Dict containing s3_locations, module, plot, dataset_id,
                theme, and component_id for plot generation.
            existing_trace_metadata: Previously stored trace metadata to detect
                if re-rendering is unnecessary.

        Returns:
            Tuple of (figure, trace_metadata) or (no_update, no_update) if
            already rendered or no trigger data.
        """
        ...
        if existing_trace_metadata and existing_trace_metadata.get("summary"):
            logger.debug("Already rendered, skipping re-render")
        ...
        try:
            ...
            logger.info(f"Rendering MultiQC plot: {module}/{plot} (theme={theme})")
            logger.debug(f"S3 locations: {s3_locations}")

            # Validate required parameters
            if not s3_locations:
                logger.error("s3_locations is empty")
                return _create_error_figure(
                    "Error: No S3 locations", "Missing S3 locations for MultiQC data"
                ), {}

            if not module:
                logger.error("module is missing")
                return _create_error_figure(
                    "Error: No module specified", "MultiQC module not specified"
                ), {}

            if not plot:
                logger.error("plot is missing")
                return _create_error_figure(
                    "Error: No plot specified", "MultiQC plot name not specified"
                ), {}
            ...
            # Analyze plot structure for filtering support
            trace_metadata = analyze_multiqc_plot_structure(fig)

            trace_count = len(fig.data) if hasattr(fig, "data") else 0
            logger.info(f"MultiQC plot rendered successfully with {trace_count} traces")

            return fig, trace_metadata

        except Exception as e:
            logger.error(
                f"Error rendering MultiQC plot: {type(e).__name__}: {str(e)}", exc_info=True
            )
            return _create_error_figure(
                "Error Rendering MultiQC Plot",
