"""
Shared save utilities for interactive components.

This module provides reusable functions for saving interactive component metadata
...
def save_interactive_to_dashboard(
    dashboard_id: str, component_metadata: dict, token: str, app_prefix: str = "dashboard"
) -> str:
    """
    Save or update interactive component in dashboard.

    This function handles the complete API interaction for saving interactive metadata:
    ...
    Args:
        ...
        component_metadata: Complete component metadata dict with all interactive fields
    ...
    logger.info(f"💾 SAVE INTERACTIVE - Dashboard: {dashboard_id}, Component: {component_id}")
    ...
    try:
        ...
        logger.info(f"✅ SAVE INTERACTIVE SUCCESS - Component {component_id} saved")
        logger.info(f"   API Response status: {update_response.status_code}")

    except Exception as e:
        logger.error(f"❌ SAVE INTERACTIVE FAILED - Error: {e}")
