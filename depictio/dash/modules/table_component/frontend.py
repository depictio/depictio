...
def register_callbacks_table_component(app):
    """Register all callbacks for the table component system."""
    ...
    @app.callback(
        ...
        [
            ...
            Input({"type": "table-update-trigger", "index": MATCH}, "data"),
        ],
        [
    ...
    def update_table_component_direct(
        trigger, table_trigger, component_meta, local_data, metadata, pathname
    ):
        """Update table component directly - part of modular draggable system."""
        ...
        logger.info(f"🔄 Updating table component {meta_index} directly")

        try:
            # Build the table component using existing build_table function
            updated_component = build_table(
                index=meta_index, stored_metadata=metadata, TOKEN=TOKEN, dashboard_id=dashboard_id
            )

            if updated_component:
                logger.info(f"✅ Table component {meta_index} updated successfully")
                return updated_component
            else:
                logger.warning(f"⚠️ Failed to build table component {meta_index}")
                return dash.no_update

        except Exception as e:
            logger.error(f"❌ Error updating table component {meta_index}: {e}")
            return dash.no_update
