describe('Create and manage dashboard', () => {
    ...
    it('logs in, creates, duplicates, edits, toggles privacy and manages dashboards', () => {
        ...
        // Find the newly created dashboard by looking for any Test Dashboard
        cy.contains('Test Dashboard').should('be.visible');

        // For simplicity, just verify dashboard creation and clean up
        cy.wait(2000)

        // Clean up - delete any test dashboards
        cy.get('body').then(($body) => {
            if ($body.find(':contains("Test Dashboard")').length > 0) {
