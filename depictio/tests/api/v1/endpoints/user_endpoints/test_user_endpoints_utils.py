...
class TestCheckPassword:
    ...
    def setup_method(self, method):
        # Set up patches
        self.find_user_patcher = patch("depictio.api.v1.endpoints.user_endpoints.utils.find_user")
        self.mock_find_user = self.find_user_patcher.start()
        
        self.verify_password_patcher = patch("depictio.api.v1.endpoints.user_endpoints.utils.verify_password")
        self.mock_verify_password = self.verify_password_patcher.start()
        
    
    def teardown_method(self, method):
        # Stop all patches
        for patcher_attr in ['find_user_patcher', 'verify_password_patcher', 'logger_patcher']:
            if hasattr(self, patcher_attr):
                getattr(self, patcher_attr).stop()
    
    def test_check_password_user_found_password_correct(self):
        ...
        # Configure mocks
        self.mock_find_user.return_value = mock_user
        self.mock_verify_password.return_value = True
        
        # Act
        result = self.check_password(test_email, test_password)
        
        # Assert
        self.mock_find_user.assert_called_once_with(test_email)
        self.mock_verify_password.assert_called_once_with(mock_user.password, test_password)
        assert result is True
    
    def test_check_password_user_found_password_incorrect(self):
    ...
    def test_check_password_exception_handling(self):
        ...
        # Configure find_user to raise an exception
        self.mock_find_user.side_effect = Exception("Database error")
        
        # Act & Assert
        with pytest.raises(Exception) as exc_info:
            self.check_password(test_email, test_password)
...
class TestCreateGroupHelper:
    ...
    def setup_method(self, method):
        ...
        self.convert_objectid_patcher = patch("depictio_models.models.base.convert_objectid_to_str", 
    ...
    def teardown_method(self, method):
        ...
        # Stop all patches
        for patcher_attr in ['convert_objectid_patcher', 'logger_patcher', 'ensure_mongodb_patcher']:
            if hasattr(self, patcher_attr):
                getattr(self, patcher_attr).stop()
    ...
    def test_create_group_exception(self):
        ...
        self.groups_collection.insert_one = MagicMock(side_effect=Exception("Test error"))
