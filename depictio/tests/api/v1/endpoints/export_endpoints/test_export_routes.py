...
class TestSecurityHeaderExemption:
    ...
    def test_html_embed_304_escapes_the_baseline_headers(self, secured_client, mongo):
        """A 304 carries no Content-Type, so the exemption cannot key off it alone.

        RFC 9110 has a 304's headers *replace* the stored ones, so a baseline
        `X-Frame-Options: SAMEORIGIN` here retro-fits itself onto the embed
        already in the browser cache: the first view of a frame works and every
        revalidated one after it goes blank.
        """
        first = secured_client.get(_url("txt-1", format="html"))
        etag = first.headers["etag"]

        response = secured_client.get(_url("txt-1", format="html"), headers={"If-None-Match": etag})

        assert response.status_code == 304
        assert "X-Frame-Options" not in response.headers
        assert "content-security-policy" not in response.headers, (
            "a 304 must leave the cached embed's own CSP in place"
        )

    def test_json_export_304_keeps_the_baseline_headers(self, secured_client, mongo):
        # `*` matches any representation, so the 304 is reached without having to
        # build a real figure first just to learn its ETag.
        response = secured_client.get(_url("fig-1", format="json"), headers={"If-None-Match": "*"})
