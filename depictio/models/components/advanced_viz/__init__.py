...
Four composite, analysis-grade viz kinds (volcano, embedding, manhattan,
stacked_taxonomy), each bundling a chart with builtin filters/controls.
At the outer dashboard level these are a single ``component_type =
"advanced_viz"``; per-kind config is a Pydantic discriminated union by
``viz_kind`` (see configs.py).
