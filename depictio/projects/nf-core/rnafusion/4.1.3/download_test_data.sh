#!/usr/bin/env bash
# Download the nf-core/rnafusion 4.1.3 AWS megatest subset for local testing.
...
#   Default TARGET_DIR: ~/Data/depictio-nfcore/rnafusion/4.1.3/megatest
set -euo pipefail
exec python3 "$(dirname "$0")/../../../../../scripts/nfcore_megatest.py" fetch \
  --pipeline rnafusion --version 4.1.3 \
  --dest "${1:-$HOME/Data/depictio-nfcore/rnafusion/4.1.3/megatest}"
