...
printf '%s\n' "$DASH_FILES" | while IFS=$'\t' read -r title file; do
    ...
    # committed seed empty on any error. Stage, validate, then move.
    if ! python3 -c 'import json,sys; json.load(open(sys.argv[1]))' "${out_file}.tmp" 2>/dev/null; then
        echo "ERROR: export for ${title} is not valid JSON, keeping the committed seed" >&2
        rm -f "${out_file}.tmp"
        exit 1
    fi
...
# DEPICTIO_CONTEXT=cli: importing db_init_reference_datasets builds Settings(),
# whose _enforce_server_secrets rejects the dev-default MinIO password in
# server context. This script only reads STATIC_IDS, so the client context
# is the accurate one and the check is skipped by design.
DEPICTIO_CONTEXT=cli python "$SCRIPT_DIR/remap_seeds_to_static_ids.py"
