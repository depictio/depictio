#!/usr/bin/env bash
# Download the nf-core/mag 5.4.2 AWS megatest subset for local testing.
...
#   Default TARGET_DIR: ~/Data/depictio-nfcore/mag/5.4.2/megatest
set -euo pipefail
exec python3 "$(dirname "$0")/../../../../../scripts/nfcore_megatest.py" fetch \
  --pipeline mag --version 5.4.2 \
  --dest "${1:-$HOME/Data/depictio-nfcore/mag/5.4.2/megatest}"
