#!/usr/bin/env bash
# Download the nf-core/nanoseq 3.0.0 AWS megatest subset for local testing.
...
#   Default TARGET_DIR: ~/Data/depictio-nfcore/nanoseq/3.0.0/megatest
set -euo pipefail
exec python3 "$(dirname "$0")/../../../../../scripts/nfcore_megatest.py" fetch \
  --pipeline nanoseq --version 3.0.0 \
  --dest "${1:-$HOME/Data/depictio-nfcore/nanoseq/3.0.0/megatest}"
