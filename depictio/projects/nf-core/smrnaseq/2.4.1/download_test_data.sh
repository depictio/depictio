#!/usr/bin/env bash
# Download the nf-core/smrnaseq 2.4.1 AWS megatest subset for local testing.
...
#   Default TARGET_DIR: ~/Data/depictio-nfcore/smrnaseq/2.4.1/megatest
set -euo pipefail
exec python3 "$(dirname "$0")/../../../../../scripts/nfcore_megatest.py" fetch \
  --pipeline smrnaseq --version 2.4.1 \
  --dest "${1:-$HOME/Data/depictio-nfcore/smrnaseq/2.4.1/megatest}"
