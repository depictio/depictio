#!/usr/bin/env bash
# Download the nf-core/sarek 3.10.0 AWS megatest subset for local testing.
...
#   Default TARGET_DIR: ~/Data/depictio-nfcore/sarek/3.10.0/megatest
set -euo pipefail
exec python3 "$(dirname "$0")/../../../../../scripts/nfcore_megatest.py" fetch \
  --pipeline sarek --version 3.10.0 \
  --dest "${1:-$HOME/Data/depictio-nfcore/sarek/3.10.0/megatest}"
