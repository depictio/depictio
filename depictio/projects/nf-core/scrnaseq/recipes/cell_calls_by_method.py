...
def transform(sources: dict[str, pl.DataFrame]) -> pl.DataFrame:
    caller_sets = {
        "cellranger": _barcode_core_set(sources.get("cellranger"), "cellranger"),
        "cellranger_cellbender": _barcode_core_set(
            sources.get("cellranger_cellbender"), "cellranger_cellbender"
        ),
        "simpleaf_cellbender": _barcode_core_set(
            sources.get("simpleaf_cellbender"), "simpleaf_cellbender"
        ),
        "kallisto": _barcode_core_set(sources.get("kallisto"), "kallisto"),
        "kallisto_cellbender": _barcode_core_set(
            sources.get("kallisto_cellbender"), "kallisto_cellbender"
        ),
    }
    ...
    all_barcodes = sorted(set().union(*caller_sets.values()))
    if not all_barcodes:
        raise ValueError("cell_calls_by_method: no caller produced any barcode")

    df = pl.DataFrame({"barcode_core": all_barcodes})
