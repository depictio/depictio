#!/usr/bin/env bash
# Download the nf-core/scrnaseq 4.2.0 AWS megatest subset for local testing.
...
#   Default TARGET_DIR: ~/Data/depictio-nfcore/scrnaseq/4.2.0/megatest
set -euo pipefail
exec python3 "$(dirname "$0")/../../../../../scripts/nfcore_megatest.py" fetch \
  --pipeline scrnaseq --version 4.2.0 \
  --dest "${1:-$HOME/Data/depictio-nfcore/scrnaseq/4.2.0/megatest}"
