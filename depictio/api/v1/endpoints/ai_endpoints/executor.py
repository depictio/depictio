...
def _validate_call(call: ast.Call) -> None:
    ...
    if isinstance(func, ast.Attribute):
        ...
        # name against the combined allowlist is sufficient here. Direct
        # pl.<helper>() calls are held to the tighter pl-only list.
        ...
        elif func.attr not in ALLOWED_DF_METHODS and func.attr not in ALLOWED_PL_FUNCS:
            raise ValidationError(
        ...
        if not isinstance(node, ast.Name):
            raise ValidationError(
                reason="call chain does not root at df/pl",
                node_lineno=call.lineno,
            )
        if node.id not in ("df", "pl"):
            raise ValidationError(
