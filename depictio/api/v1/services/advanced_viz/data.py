...
Unlike the route, this never samples. The route reduces a frame to something a
browser can draw, and reports the reduction so the renderer can badge it; an
export is a published artefact that a reader cannot interrogate, so it is exact
or it says so. The only bound is the full-load ceiling, and reaching it sets
``exact: False``.
"""
...
def load_tidy_columns(
    *,
    ...
    ``limit_rows`` defaults to the full-load ceiling
    (``performance.figure_max_load_rows``) rather than to a display cap: this
    feeds exports, and a silently truncated export is worse than a large one.

    Returns:
        ``{"columns", "rows", "row_count", "filter_applied", "exact"}``.
        ``columns`` echoes only the requested columns that survived projection —
        a caller that bound an optional column the recipe never emitted gets it
        omitted rather than an error. ``exact`` is False when the frame reached
        the ceiling, i.e. rows exist that this payload does not carry.
    """
    ...
    ceiling = (
        int(settings.performance.figure_max_load_rows) if limit_rows is None else int(limit_rows)
    )
    ...
    return {
        ...
        # Reaching the ceiling means rows were left behind. The caller surfaces
        # this; an export that quietly drops rows is indistinguishable from a
        # complete one.
        "exact": int(df.height) < ceiling,
    }
