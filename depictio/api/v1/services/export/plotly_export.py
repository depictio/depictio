...
from depictio.api.v1.services.trace_rendering import svg_equivalent
...
def _prefer_svg_traces(spec: dict[str, Any]) -> dict[str, Any]:
    """Downgrade every GPU trace in an outgoing spec to its SVG equivalent.

    Applied at the boundary rather than in each builder, because that is the
    only place that knows the spec is leaving. The advanced-viz kind builders
    could decide it themselves — export is their only caller — but the figure,
    map and MultiQC specs come from the same code paths the live viewer renders
    through, which have no idea whether this figure is about to be drawn in our
    dashboard or in a stranger's page.

    Only ever gl -> svg. ``scatter`` supports a superset of ``scattergl``'s
    attributes, so dropping to it cannot invalidate a trace; the reverse could.
    A type with no SVG form (``scatter3d``, ``surface``) is left alone —
    see services/trace_rendering.py.
    """
    for trace in spec.get("data") or []:
        if not isinstance(trace, dict):
            continue
        svg = svg_equivalent(trace.get("type") or "")
        if svg:
            trace["type"] = svg
    return spec
...
def _python_advanced_viz_spec(
    ...
    spec = build(kind, config=config, rows=tidy["rows"], theme=theme, controls=controls)
    if notes is not None:
        notes["row_count"] = tidy.get("row_count")
        notes["complete"] = bool(tidy.get("exact", True))
    return spec
...
async def build_plotly_export(
    *,
    ...
    # Filled in by the builders that know how much of the frame they read. A
    # spec that silently left rows behind is indistinguishable from a complete
    # one once it is on someone else's page, so it has to be said in `meta`.
    notes: dict[str, Any] = {}
    ...
    if component_type == "figure":
        ...
        if source == "celery":
            ...
            spec = _python_advanced_viz_spec(
                component, filter_metadata, str(viz_kind), theme, controls, notes=notes
            )
    ...
    return json_safe(
        {
            **_prefer_svg_traces(spec),
            ...
            "meta": {
                ...
                # Absent unless the builder counted rows. `complete: false` means
                # the source frame hit the full-load ceiling and this spec draws
                # a prefix of it.
                **({"row_count": notes["row_count"]} if "row_count" in notes else {}),
                **({"complete": notes["complete"]} if "complete" in notes else {}),
