...
def _add_advanced_viz(data, component, synthetic_dc_id, filters, viz_kind, user) -> None:
    ...
    # `embedding` is only Celery-backed when it is asked to compute one.
    # EmbeddingRenderer gates on exactly this key (`liveMode`), and without it
    # reads pre-computed coordinate columns straight from the DC instead — so
    # shipping a compute result there strands the renderer with no tidy data.
    if viz_kind == "embedding" and not config.get("compute_method"):
        celery_kinds.pop("embedding")
