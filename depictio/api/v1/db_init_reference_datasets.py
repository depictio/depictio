...
STATIC_IDS = {
    ...
    # Seed JSONs under ``.db_seeds/`` are generated by running the depictio
    # CLI ingest against a viralrecon test-data run; see
    # ``depictio/projects/nf-core/viralrecon/3.0.0/generate_seeds.sh``.
    "viralrecon": {
        "project": "746b0f3c1e4a2d7f8e5b9ca2",
        "workflows": {"viralrecon": "746b0f3c1e4a2d7f8e5b9c01"},
        "data_collections": {
        ...
        "dashboards": {
            ...
            "viralrecon_multiqc": "746b0f3c1e4a2d7f8e5b9ca2",
            "viralrecon_coverage_depth": "746b0f3c1e4a2d7f8e5b9cb5",
            "viralrecon_lineage_clustering": "746b0f3c1e4a2d7f8e5b9cb3",
            "viralrecon_variants": "746b0f3c1e4a2d7f8e5b9cb4",
...
async def create_reference_datasets(
    ...
    # Create reference datasets (iris, penguins, ampliseq, advanced_viz_showcase, viralrecon).
    # Viralrecon's recipe data (4.7 GB nf-core test_illumina output) is too large
    # to vendor in the repo; the project is registered so its static IDs reserve
    # the slot, but DCs without pre-computed seed files are silently dropped by
    ...
    for dataset_name in datasets_to_create:
        ...
        try:
            ...
            # Reference data is best-effort: viralrecon in particular depends on
            # locally-generated canonical TSVs that won't exist in fresh dev
            # checkouts. Skip the failing dataset rather than crashing the API.
            logger.warning(
                f"Skipping reference dataset '{dataset_name}' — initialization failed: {exc}"
            )
            continue
