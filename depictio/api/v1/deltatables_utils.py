...
# Predicate semantics live in the FastAPI-free models package so the
# serverless producer builds the exact same expressions (see
# depictio/models/components/predicates.py). Re-imported here so existing
# callers (and tests) keep importing them from this module unchanged.
from depictio.models.components.predicates import (  # noqa: F401  (re-exported)
    LINK_NO_MATCH,
...
# NOTE: ``LINK_NO_MATCH`` and ``_categorical_predicate`` moved to
# ``depictio.models.components.predicates`` (imported at the top of this
# module) so the serverless producer shares the exact same predicate
# semantics. Their behaviour is unchanged.
...
def add_filter(
    ...
    if interactive_component_type == LINK_NO_MATCH:
        ...
        if value and isinstance(value, list) and len(value) == 2:
            ...
            try:
                # Pure expression construction (ISO boundary parsing, the
                # Date/Datetime/Utf8-tolerant coalesce column expression, and
                # tz normalisation on both sides) lives in
                # ``depictio.models.components.predicates``; only the payload
                # validation and error logging stay here.
                filter_list.append(datetime_range_predicate(column_name, value[0], value[1]))
