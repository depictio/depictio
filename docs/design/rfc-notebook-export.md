...
The client also works inside a `.qmd` `{python}` block. Server-side rendering with the `quarto`
binary stays Phase 3 of the product plan: executing a generated notebook is arbitrary code
execution and needs a sandboxing design, not a packaging change.
