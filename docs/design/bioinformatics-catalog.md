...
| **Auto-mapping** | `schemas.py` → `suggest_viz_kinds()` | Runtime reverse lookup from a DC's `{col: dtype}` schema. Three kinds of evidence, each returned as `match` with short `reasons`: **named** (every distinctive role of a domain kind has a column named like one of its aliases; identity roles such as a sample or feature id do not count, and a partial match stays under the recommended bar), **shape** (generic kinds read the table's shape: a coordinate pair such as `dim_1/dim_2`, `pc1/pc2`, `*_x/*_y` for `scatter_xy`, a reduction pair for `embedding`, four or more numeric measurement axes for `parallel_coordinates`; role-less kinds pass their structural gates) and **context** (the builder sends the selection columns of the target tab, and a `record_card` is recommended only when the DC holds one of them and has enough fields to show). The numeric `score` only ranks; the builder's picker shows the match as a badge with the reasons in a tooltip. Wired into the API (`/datacollections/viz-suggestions`, optional repeatable `selection_columns` / `existing_kinds` query params) and the React DC card's "Suggested visualisations" chips. `suggest_producers()` was removed; the API still returns `producers: []` for client compatibility. |
