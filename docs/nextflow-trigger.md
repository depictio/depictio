...
![from the last task to a link](images/v1.4/nextflow/schema_trigger_flow.png)

Three ways to load the handler, three reasons for it to return without a word,
and the one thing it never writes to. The drawing is generated by
`dev/diagrams/nextflow_trigger.py`, so it can be corrected in a diff when the
flow changes.
