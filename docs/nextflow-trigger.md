...
| `depictio.config` | The onComplete snippet | **Never**, normally. `$(depictio-cli config nextflow)` points at the one inside the installed CLI, so it stays in step with it |

Copy the snippet only when you genuinely cannot reach that one:
