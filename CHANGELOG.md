...
```plaintext
ghcr.io/depictio/depictio:1.0.1-b3
...
<details>
<summary>Click to expand the changelog for 1.0.1-b3</summary>

### Changes 📜

#### New Features ✨

* feat: add DC linking UI and MultiQC DC creation via UI [9ea497a7]
...
* fix: avoid shell injection and bad parameter expansion in release workflow ([#790](https://github.com/depictio/depictio/pull/790)) [a56b300c]
* fix: screenshot debounce wrong path + Sign In shown in public mode ([#784](https://github.com/depictio/depictio/pull/784)) [cfeed8f7]
* fix: cross-DC filter target_column=None + Add-link modal polish ([#776](https://github.com/depictio/depictio/pull/776)) [d34b6816]
* fix: handle aggregation_columns_specs as list of dicts in Cytoscape [02bf6899]
* fix: extract DC columns from enriched aggregation specs for Cytoscape [e75b7d03]
* fix: resolve Cytoscape link edge referencing nonexistent column nodes [886984fb]
* fix: skip scan step for MultiQC DC creation and fix file naming [f5f50011]
...
* refactor: remove depictio/dash/ — Dash → React migration complete ([#770](https://github.com/depictio/depictio/pull/770)) [d55287d9]
* style: ruff format db_init_reference_datasets [36004203]
...
* ci: add monthly Dependabot dependency-update health-check [f61f223c]
* ci: fix dropped "Verify users" step header from debug-logging edit [90177b34]
* ci: narrow build-images PR trigger to build-recipe files only ([#796](https://github.com/depictio/depictio/pull/796)) [3ca46271]
* ci: add repackage-helm-chart workflow to overwrite OCI tag contents ([#772](https://github.com/depictio/depictio/pull/772)) [28064f66]
* chore: ruff format drift + remove unused var [7cb918ab]
* chore: ignore node_modules, .vscode/, generated cli configs [40ce0119]
* chore: gitignore root pnpm node_modules and depictio-react-core node_modules [15bfcce2]
* chore: refresh uv.lock, CLAUDE.md stale section, regenerated screenshots [edaa0c47]
* chore: untrack depictio/dashboards_yaml/ (instance/runtime-only) [3bedc301]
* chore: untrack AI session handoff scratch in depictio/viewer/ [d7e01613]
* chore: untrack auto-generated CLAUDE.md context + superseded .claude-code/ skills [69b5e671]
...
* docs: design proposal for advanced visualisation component family [55eca3dc]
* docs: sync claude-mem auto-generated activity logs [56068a62]

#### Other Changes 📝

* Bump version: 1.0.1-b2 → 1.0.1-b3 [e70b0357]
* ci(quickstart): poll the viewer in smoke test instead of one eager curl [f09cd72e]
* Update CHANGELOG.md for v1.0.1-b2 [3c3e414e]
* Bump version: 1.0.1-b1 → 1.0.1-b2 [70de14e0]
* fix(quickstart): let mongo start under cap_drop:[ALL] [24e64ba0]
* Update CHANGELOG.md for v1.0.1-b1 [dbb9536a]
* Bump version: 1.0.0 → 1.0.1-b1 [7721b7f1]
* fix(quickstart): keep working creds in .env.example for CI/dev compose [850876c8]
* ci(quickstart): also build arm64 for beta releases [372eafd8]
* ci(quickstart): build arm64 only for stable releases [a4a79525]
* ci(quickstart): verify admin user + iris dashboard seeded in smoke test [aed39f8f]
* ci(quickstart): add detailed sanity checks to the smoke test [13971e7d]
* ci(quickstart): smoke-test that the published compose boots with no .env [3dc1191f]
* fix(quickstart): auto-seed default admin in single-user mode [37106ee0]
* fix(quickstart): zero-config `docker compose up -d` + multi-arch images [3c249ee8]
* Update CHANGELOG.md for v1.0.0 [db939b79]
* Bump version: 1.0.0-b9 → 1.0.0 [1749d985]
* Update CHANGELOG.md for v1.0.0-b9 [89b63dc4]
* Bump version: 1.0.0-b8 → 1.0.0-b9 [0ee3c1b2]
* chore(screenshots): regenerate reference dashboard thumbnails [b801f220]
* fix(multiqc): drop matplotlib dep from general-stats colormap [d15ce24a]
* Update CHANGELOG.md for v1.0.0-b8 [5933db29]
* Bump version: 1.0.0-b7 → 1.0.0-b8 [03c2e0ab]
* test(catalog): invoke moved maintainer commands via the dev app [651ef000]
* feat(cli): add `depictio commands` grouped reference table [900caaab]
* refactor(cli): move the data link subgroup under the hidden dev group [8e4ec380]
* refactor(cli): move catalog match/compose to the hidden dev group [eb56367a]
* refactor(cli): slim the user-facing command surface [253f9610]
* fix(cli): catch typer.Exit in recipe_run so dc_ref standalone skip exits 0 [10141a31]
* chore(ci): bump the github-actions group across 1 directory with 15 updates [049a0ef3]
* chore(deps): bump the static-tooling group across 1 directory with 2 updates [f4a821e0]
* feat(cli): drop redundant Name column from catalog list [aa2e8245]
* feat(cli): clearer `catalog list` (split Tool id / Name + info hint) [1ed11375]
* feat(api): re-export admin agent config on every dev-mode boot [4120ef2f]
* feat(cli): drop the tip line from the startup banner [f3220869]
* chore(cli): bump depictio/cli deps, fix python-jose CVEs [8f4b721a]
* feat(cli): redesign startup banner with the depictio favicon [77913472]
* fix(cli): don't force ANSI on piped output (breaks backup CI grep) [b51deed7]
* refactor(cli): presentation sweep + parallel image uploads [091936ff]
* refactor(cli): improve performance, presentation & logging consistency [d8f58a14]
* fix(deltatables): accept pandas 3.0 string dtype ("str") in column specs [e3eebe18]
* fix(deps): hold redis at 5.2.1 — redis-py 8.0 starves the API event loop [de6b13a7]
* ci(debug): dump full backend logs when ingestion creates no deltatables [cd03feff]
* fix(devcontainer): drop dangling docker-compose.minio.yaml reference [4b7af98d]
* chore(deps): include data-layer majors (deltalake/pandas/polars/multiqc) [0d08b394]
* chore(deps): Phase B — bump runtime dependencies [69fda459]
* fix(devcontainer): harmonize MinIO credentials across worktree env layers ([#802](https://github.com/depictio/depictio/pull/802)) [e0259d71]
* feat(advanced-viz builder): richer bindings UX + sankey/heatmap binding fixes [0a94fa68]
* Rework advanced-viz suggestion model into graded, tolerant scoring [8332e5c5]
* chore(deps): Phase A — bump static analysis & test tooling ([#799](https://github.com/depictio/depictio/pull/799)) [11790aa9]
* Update CHANGELOG.md for v1.0.0-b7 [1d9d326f]
* Bump version: 1.0.0-b6 → 1.0.0-b7 [dbafca9a]
* fix(viewer): tab menu refresh + icon handling in tab/dashboard modals ([#795](https://github.com/depictio/depictio/pull/795)) [d40615ac]
* Remove producers.py suggestion engine + migrate ampliseq complex_heatmap tiles to use: ([#797](https://github.com/depictio/depictio/pull/797)) [edcbdb84]
* fix(screenshots): install sans-serif fonts in worker image + settle before capture ([#794](https://github.com/depictio/depictio/pull/794)) [203b6fcb]
* feat(helm): migrate MongoDB HA from Community Operator to Percona PSMDB Operator ([#792](https://github.com/depictio/depictio/pull/792)) [1f67a28b]
* Enable project export in public & demo mode ([#793](https://github.com/depictio/depictio/pull/793)) [4ae63284]
* Update CHANGELOG.md for v1.0.0-b6 [a18e98fc]
* Bump version: 1.0.0-b5 → 1.0.0-b6 [4ffb2bc9]
* fix(auth): multi-replica key-wipe race breaking dev-demo ingestion + React default-thumbnail fallback ([#791](https://github.com/depictio/depictio/pull/791)) [f6c8015e]
* Bump version: 1.0.0-b4 → 1.0.0-b5 [34b2df74]
* Helm chart: viewer startup order, MongoDB Community Operator HA, resource rebalance ([#788](https://github.com/depictio/depictio/pull/788)) [3f706bae]
* Bump version: 1.0.0-b3 → 1.0.0-b4 [673f98df]
* fix(demo): restore public reference dashboards + keep temp users for the session ([#789](https://github.com/depictio/depictio/pull/789)) [aa21cc75]
* Parallel React frontend + Playwright e2e (auth + dashboards-management slice) ([#782](https://github.com/depictio/depictio/pull/782)) [0bc881a0]
* feat(catalog): module-granular bioinformatics tool→viz catalog (nf-core/bio.tools/EDAM) ([#783](https://github.com/depictio/depictio/pull/783)) [a627784f]
* Bump version: 1.0.0-b2 → 1.0.0-b3 [7c8ec4a8]
* fix(rate-limit): use X-Real-IP instead of TCP source for per-user buckets ([#787](https://github.com/depictio/depictio/pull/787)) [fe323625]
* Revert "fix(rate-limit): use X-Real-IP instead of TCP source for per-user buckets" [a9b24541]
* fix(rate-limit): use X-Real-IP instead of TCP source for per-user buckets [9cead7a6]
* fix(helm): align celery worker image tag to 1.0.0-b2 [be915031]
* Update CHANGELOG.md for v1.0.0-b2 [227b7d69]
* Bump version: 1.0.0-b1 → 1.0.0-b2 [843b709a]
* fix(settings): relax password constraints for usability ([#786](https://github.com/depictio/depictio/pull/786)) [d19fbef7]
* Remove -beta suffix from React routes (Dash→React cutover) ([#785](https://github.com/depictio/depictio/pull/785)) [f25f03be]
* Update CHANGELOG.md for v1.0.0-b1 [4d616ed1]
* fix(bump): correct SKIP hook ID after dash removal [0bab4634]
* Bump version: 0.13.12 → 1.0.0-b1 [ff96f99c]
* sec(pr1): land CRITICAL + HIGH security fixes (excl. cookie migration) ([#779](https://github.com/depictio/depictio/pull/779)) [3ca5f91e]
* Fix tab three-dots menu triggering URL refresh in React viewer [63324dd7]
* Update CHANGELOG.md for v0.13.12 [99ad1504]
* Bump version: 0.13.11 → 0.13.12 [2d25a37c]
* fix(filters): cross-DC link filter uses join column, not filter column ([#777](https://github.com/depictio/depictio/pull/777)) [0ac305dd]
* Update CHANGELOG.md for v0.13.11 [1537c445]
* Bump version: 0.13.10 → 0.13.11 [55f3cbb9]
* Update CHANGELOG.md for v0.13.10 [2ebf6988]
* Bump version: 0.13.9 → 0.13.10 [a43e1e0a]
* fix(builder): viz suggestion precision, text-step skip, screenshot regen on Save ([#775](https://github.com/depictio/depictio/pull/775)) [592f9188]
* Update CHANGELOG.md for v0.13.9 [315b7079]
* Bump version: 0.13.8 → 0.13.9 [9e7e4695]
* fix(auth): let admins bypass public/demo-mode write-action gates ([#774](https://github.com/depictio/depictio/pull/774)) [c11a794f]
* Update CHANGELOG.md for v0.13.8 [02415436]
* Bump version: 0.13.7 → 0.13.8 [b24c7bd8]
* Fix React project import 404 and per-project creation timestamps ([#773](https://github.com/depictio/depictio/pull/773)) [5bbe89a4]
* fix(screenshots): honour dashboard-level owners after duplication ([#771](https://github.com/depictio/depictio/pull/771)) [6fd87c6b]
* Update CHANGELOG.md for v0.13.7 [7bd55279]
* Bump version: 0.13.6 → 0.13.7 [49962549]
* fix(deltatables): use positional indexing in precompute_columns_specs ([#769](https://github.com/depictio/depictio/pull/769)) [c5f7af62]
* Update CHANGELOG.md for v0.13.6 [172f24fd]
* Bump version: 0.13.5 → 0.13.6 [4b492162]
* fix(seeds): force format=tsv on recipe DC conversion ([#768](https://github.com/depictio/depictio/pull/768)) [fccb1004]
* Update CHANGELOG.md for v0.13.5 [1e2e29b8]
* Bump version: 0.13.4 → 0.13.5 [aee046ea]
* feat(viralrecon): bundle 15 canonical seed TSVs + missing recipes + CI check ([#767](https://github.com/depictio/depictio/pull/767)) [9400824c]
* Update CHANGELOG.md for v0.13.4 [f8b852a0]
* Bump version: 0.13.3 → 0.13.4 [c00a1d2a]
* feat(viralrecon): bundle minimal scan-target test data into image ([#766](https://github.com/depictio/depictio/pull/766)) [e293e51f]
* Update CHANGELOG.md for v0.13.3 [4cb1b2ac]
* Bump version: 0.13.2 → 0.13.3 [eba8199b]
* test(seeds): assert all reference-dashboard dc_ids match STATIC_IDS [98f63f4a]
* fix(seeds): viralrecon dashboards use static DC IDs + invariant test ([#765](https://github.com/depictio/depictio/pull/765)) [5110ca31]
* Update CHANGELOG.md for v0.13.2 [5facb5c6]
* Bump version: 0.13.1 → 0.13.2 [691c1083]
* fix(viewer): enable dashboard pinning in public/demo mode [a12461f5]
* fix(ampliseq): add metadata→multiqc_data DCLink to 2.16.0 template [7429ddb1]
* fix(cli): process materialised recipe DCs as file scans [61d23f1d]
* fix(seeds): preserve resolved DATA_ROOT for viralrecon [089606d2]
* fix(helm): set imagePullPolicy=Always on busybox/curl init containers [731cb336]
* Update CHANGELOG.md for v0.13.1 [f01a6d2b]
* Bump version: 0.13.0 → 0.13.1 [73d83f08]
* feat(card-builder): live preview for top_n / coverage / concentration [78a31a88]
* fix(card-builder): Mantine 7 group format for Multi-metric select [58370a66]
* fix(seeds): preserve source=transformed on recipe DCs after materialisation [b6220e2b]
* fix(card): persist multi-metric config + guard against missing outliers [90324f44]
* fix(dashboards): duplicate child tabs alongside the main tab [7b9d19db]
* fix(screenshot): honour configured capture timeout on element / page screenshot [317cd2d8]
* feat(viewer): surface template-origin chip on /dashboards-beta + settings drawer [cc3e2555]
* feat(auth): hand off session across Dash → React beta origin boundary [f7e68802]
* chore(screenshots): refresh ampliseq tab thumbnails [dcbf7b59]
* fix(ampliseq): wire alpha-diversity habitat filter to rarefaction viz [8b7db988]
* feat(admin-beta): include all seed projects in maintenance cleanup [e95a996f]
* feat(admin-beta): list all seed dashboards under each project [dd2bc1f6]
* fix(viewer): date picker range selection + interactive reset state [ae546e20]
* fix(multiqc): wire cross-DC filter resolution from metadata DC [b9f7d7fc]
* feat(advanced-viz): producer fingerprints for nf-core column shapes [b9abfb4c]
* chore(screenshots): refresh seed dashboard thumbnails + add viralrecon [10eed956]
* refactor(admin-beta): replace project badge cluster with bullet list [7c91abae]
* feat(viewer): make dashboard cards and sidebar tabs proper anchor links [a1471d27]
* feat(viewer): regroup /dashboards-beta into nf-core + demo sections [7727c4a3]
* feat(viralrecon): register nf-core/viralrecon 3.0.0 as seeded reference dataset [b561ff8e]
* refactor(advanced-viz): drop overview tab, rename to Advanced Visualisations [a6df9bf8]
* feat(api): add DEPICTIO_DISABLE_EXAMPLE_DASHBOARDS env var to gate seeding [a6ec6aea]
* feat(walkthrough): add DEPICTIO_WALKTHROUGH_DISABLED kill switch [a118c5b0]
* fix(walkthrough): gate on /auth, DEPICTIO_DEV_MODE, and screenshots [9524c6f1]
* Update CHANGELOG.md for v0.13.0 [d6f3d157]
* Bump version: 0.12.2 → 0.13.0 [68e30294]
* ci(ampliseq): download phylogenetic tree + taxonomy + ASV table [f6dcdc03]
* fix(ci): align iris dashboard title with seed after polish commit [ff571c4e]
* test(deltatables): mock collect_schema for filter-applied path [4d87e311]
* merge: resolve conflicts with main [d29572d4]
* refactor(viewer,api): apply reviewer + simplifier feedback from PR review [bcd57b87]
* fix(ampliseq): propagate cross-DC filters to figures/tables/advanced viz [72f54e94]
* feat(viewer): move Reset to filters panel header [60f6cd15]
* chore(showcase): iris/penguins dashboard polish [d5a3b7ba]
* chore(showcase): explicit layouts on iris/penguins YAML + regen script [e47396de]
* chore(screenshots): regenerate dashboard screenshots + auto-gen CLAUDE.md update [e81d5b80]
* feat(dev): nfcore-megatests showcase init project (advanced-viz testbed) [e41ed680]
* chore(dev): /import-template + /reseed slash commands, reseed script, fixtures docs [13e7bb2f]
* chore(deps): bump viewer deps (mantine tiptap 7.17 + tiptap 2.27) [dfb61a79]
* feat(builder): viz suggestions in project detail + card builder + DC pre-filter [2d82dfc0]
* feat(viralrecon): variant/coverage canonical recipes + dashboard restructure [cc823416]
* feat(ampliseq): funnel restructure (alpha-diversity/ordination/phylogeny tabs) + canonical recipes [7b7db7cc]
* feat(coverage-track): genome annotation overlay (GFF) for SARS-CoV-2/HIV/HBV/RSV [6e45f03b]
* feat(advanced-viz): palette overrides, Manhattan controls, col-annotation strips [69de7606]
* feat(api): expose table DC append / replace / clear endpoints [4c40708d]
* feat(advanced-viz): producer registry + viz-kind suggestion engine [e769d32c]
* feat(components): text tiles + multi-metric card layouts (box_plot/top_n/coverage/concentration) [f80adccf]
* fix(advanced-viz): merge filter columns into projection before runtime filters [5032947f]
* fix(io): R-emitted NA handling + MinIO single-part PUT + deltalake>=0.24 fallback [4fb01350]
* Update CHANGELOG.md for v0.12.2 [86921c82]
* Bump version: 0.12.1 → 0.12.2 [8fc85054]
* feat(viewer): gate figure Code Mode behind ownership + deployment mode [1f15d7cb]
* fix(viewer): cast Plotly selection-clear payloads for strict types [1c44f785]
* fix(viewer): add @types/plotly.js so CI tsc finds plotly types [ae674ec7]
* fix(viewer): declare 'plotly.js' module so tsc accepts the import [e2bd8056]
* chore(screenshots): refresh dashboard light thumbnail [41655108]
* fix(figure): drop heatmap+scatter_matrix, filter unsupported px kwargs [464235e2]
* fix(figure): advanced viz types no longer silently downgrade to scatter [8975a691]
* fix(viewer): cross-filtering only for scatter / scatter_3d visus [3392239e]
* docs(viewer): drop the threat-model section from About Code Mode [52fea136]
* fix(viewer): single-line layout for pre-loaded modules [b1b3432e]
* feat(viewer): code-mode help — module table, threat-model, columns-as-table [41307376]
* feat(viewer): code-mode polish — under-preview cross-filter, resizable editor, collapsible help [bb62bdfa]
* feat(viewer): mark joined DCs in component-builder dropdown + info card [f6600f31]
* feat(viewer): order interactive options by availability, then natural sort [b646e278]
* fix(viewer): match read-pair suffixes when intersecting multiqc samples [3734d411]
* fix(viewer): always include filter source DC in available-values intersection [a6e8c4f9]
* fix(api): multiqc sample-mappings falls back to canonical_samples [b1153005]
* fix(viewer): derive project_id from stored_metadata, ignore empty available sets [7ae6a3df]
* feat(viewer): availableValues now intersects multiqc samples with table DCs [59dae5a7]
* fix(viewer): reset actually clears Plotly lasso/box (use imported Plotly, gate on transition) [af649d01]
* fix(viewer): alias bare plotly.js to dist/plotly in vite config [76b120fa]
* fix(viewer): use window.Plotly handle instead of importing plotly.js [5cb5baf2]
* fix(viewer,cli): persistent reset icon, clear Plotly selection, join fan-out check [c8f34586]
* fix(viewer): reset-button styling, builder section parity, auth/data fixes [dedcd539]
* feat(viewer): filter source badge, cross-filter toggle parity, available-values greying [bb30c39e]
* perf(screenshots): switch auto-screenshot to React SPA, ~2× faster than Dash [2e99e9d1]
* feat(api,dev): React-beta screenshot endpoint + advanced-viz docs capture pipeline [1d09d0cc]
* feat(advanced-viz): merge ancombc_differentials into da_barplot [e24fb123]
* fix(ci): register plotly-upset pixi source + reformat fixtures script [d5242212]
* feat(advanced-viz): MA/Lollipop/Dot/QQ/Sunburst/Oncoplot renderers + Show-data filter fixes [2b6968f9]
* feat(showcase): UpSet plot dashboard tab [e10c0fd7]
* feat(advanced-viz): React UpsetRenderer + dispatch wiring [dc3eb62d]
* feat(advanced-viz): dispatch/poll endpoints for UpSet + register dashboard [2007ed73]
* feat(advanced-viz): Celery compute_upset task [b7572ca6]
* feat(advanced-viz): backend model for UpSet plot [bd1de48d]
* chore(deps): vendor plotly-upset package + register editable source [0285f575]
* feat(showcase): ComplexHeatmap dashboard tab on the feature matrix [50f89f1e]
* feat(advanced-viz): React ComplexHeatmapRenderer + dispatch wiring [980e2f54]
* feat(advanced-viz): Celery compute_complex_heatmap task [107727f8]
* feat(advanced-viz): backend model + dispatch/poll endpoints for ComplexHeatmap [e3d72bfd]
* feat(showcase): GSEA dot-plot fixture + dashboard tab [f572ced3]
* feat(advanced-viz): React EnrichmentRenderer (dot plot) [0a400130]
* feat(advanced-viz): backend model for GSEA / GO / pathway enrichment [aa74cd94]
* feat(showcase): rarefaction + ancombc + DA barplot fixtures and dashboard tabs [d5073d7a]
* feat(advanced-viz): React renderers for rarefaction + ancombc + DA barplot [baddfb2d]
* feat(advanced-viz): backend models for rarefaction + ancombc_differentials + da_barplot [ea287333]
* feat(advanced-viz): expose cluster + color cols on feature DC so live UMAP gets Colour-by [d663e337]
* fix(advanced-viz): Show-data popover uses AG Grid Community [c0e7fc24]
* fix(chrome): wrap each extraActions child individually so vertical orientation works [3fe09b68]
* fix(advanced-viz): chrome-integrated Settings + Show-data ActionIcons [47ed0611]
* fix(advanced-viz): visible frame, fullscreen, controls + show-data popovers [5b61b339]
* feat(advanced-viz): live Celery clustering (PCA/UMAP/t-SNE/PCoA) with cache [333f18ea]
* fix(advanced-viz): titles + border, drop show-data drawer, scanpy styling, tree filter+branch-len [04d59e6a]
* feat(advanced-viz): phylogenetic tree viz (Microreact-style) [82ae444b]
* fix(clustering): stronger signal + paper-grade scatter styling [905fc960]
* fix(advanced-viz): show-data overlap + AG Grid drawer; wire density toggle [e2ab8e8f]
* fix(showcase): main-tab _id must equal project_id so child tabs surface [4267f4a8]
* feat(advanced-viz): fix Manhattan + StackedTaxonomy + add 4 clustering tabs + collapsible data table [87e6089f]
* fix(advanced-viz): pass init_data to load_deltatable_lite to avoid 401 [e4fa510b]
* fix(advanced-viz): re-export fetchAdvancedVizKinds/Data + fetchPolarsSchema from depictio-react-core [9b75d7a1]
* feat(showcase): dedicated advanced-viz demo project with 5 dashboards [72756ecb]
* feat(ampliseq): seed advanced-viz showcase dashboard + dc_ref-based recipes [0503fec5]
* feat(advanced-viz): MVP component family — volcano, embedding, manhattan, stacked taxonomy [5f6f11b4]
* docs(advanced-viz): add per-viz input schemas, compute class, Celery offload [5ffe7034]
* Update CHANGELOG.md for v0.12.1 [a5b418b4]
* Bump version: 0.12.0 → 0.12.1 [14b4bc85]
* fix(screenshots): restore correct dark variant for 646b0f3c1e4a2d7f8e5b8ca2 [4a2e0c99]
* chore(screenshots): refresh dashboard thumbnails [1f200cc0]
* feat(docs): playwright-driven docs screenshot capture for React viewer [8930f8a5]
* feat(viewer): NotesFooter read-only for non-owners [039baa95]
* fix(viewer): move notes footer toggle from bottom-left to bottom-right [8b9d58bb]
* fix(viewer): import @mantine/tiptap/styles.css so RichTextEditor renders [cf9d46c4]
* feat(viewer): hide Data Source step for text + fullscreen notes drawer [f96cd2de]
* fix(viewer): add tiptap deps to workspace pnpm-lock.yaml [a61e8758]
* feat(viewer): add text component and notes footer drawer [103752bf]
* viewer: gate edit affordances on dashboard ownership [c9cc578b]
* viewer(walkthrough): guide visitor through actual duplicate flow [914cb5ae]
* viewer(walkthrough): mention duplicate-to-temp-session upfront in welcome [a1a0ef91]
* viewer(walkthrough): add duplicate-to-temp-session step in public tour [761f68b7]
* viewer(walkthrough): public tour rewrite for shared-instance framing [84e4d6c6]
* viewer(walkthrough): tighten copy on resize + dashboard-save steps [2a9681ad]
* viewer(walkthrough): render **bold** in step body strings [1af2751f]
* viewer(walkthrough): expand component-build phase with configure + resize steps [e7728f2f]
* viewer(walkthrough): unblock target-pending state + clickable component-type [426e10ca]
* viewer(walkthrough): fix component-builder route + watch DOM with MutationObserver [c277a3ce]
* viewer(walkthrough): builder copy revisions + open-editor step [864a6f56]
* viewer(walkthrough): projects-first flow, favicon hero, race-safe auto-advance [5ec7b338]
* viewer(walkthrough): drop route guard from builder welcome step [36b784b7]
* viewer(walkthrough): mode-aware guided tour with spotlight backdrop [5a2b98dd]
* Update CHANGELOG.md for v0.12.0 [23bb3d7f]
* Bump version: 0.11.2 → 0.12.0 [70a09e11]
* chore(claude): add slash commands for bump/commit/worktree workflows [7bc3d9f1]
* feat(viewer): inline coordinates detection in Create DC modal [a73373c7]
* feat(viewer): dashboards + projects revamp (toolbar, table, pins, filters) [fdcd3eed]
* fix(viewer-dev): unbreak favicon under Vite's base prefix [62c077fe]
* fix(dev): unblock side-by-side worktree dev stacks [03129fe7]
* feat(viewer): map config rename + chrome polish + screenshot skip-on-unreachable [5137d1a2]
* chore(api): bump MultiQC preview offload timeout + collapse multi-line log [118d4061]
* feat(viewer): Coordinates tab in Create DC + inline DC previews + Map-aware builder [59ebecb9]
* feat(sdk): fetchMultiQCBuilderOptions helper + latColumn/lonColumn upload input [9fcf8a5c]
* feat(api): /create_from_upload accepts lat/lon hints with column validation [7f77fe3c]
* feat(models): DCTableCoordinatesConfig — Map-capable table DC variant [57b2ccb0]
* chore(dev): multi-run report generator — companion tables + bad-scenario fixtures [efe16dd1]
* refactor(viewer): canonical selected_* keys + SPA route polish [c26fc028]
* feat(multiqc): disk-persistent prerender store + Celery worker rebuild [e496a6b8]
* feat(viewer): polish cross-DC Link create/edit modal [7b428ae4]
* feat(viewer,api): unified Manage Data Collection modal + Table append/replace/clear [71a23727]
* feat(multiqc): uniformity validator + dry-run check endpoint + Check now button [5e9edf29]
* chore(devcontainer): wire viewer-dev Vite port through allocator [092d6622]
* feat(viewer): Create DC modal polish + MultiQC uniformity checklist [f9650392]
* feat(multiqc): run uniformity checks on the create flow [c452168e]
* fix(api): cascade-delete cross-DC links when a DC is removed [6d6b1e60]
* fix(viewer): let Vite rewrite favicon path via base [1ad68eed]
* fix(screenshots): regenerate clean example dashboard previews [d897c7e6]
* fix(viewer): hide demo banner + tour in React routes [35df5cfa]
* feat(viewer): cross-DC link CRUD + MultiQC create/manage in React [d54916b3]
* feat(multiqc): HTTP endpoints for create + append + replace from React [1ab41b0e]
* fix(multiqc): cache key correctness + safer append + helper tests [295743eb]
* fix(multiqc): authorize destructive endpoints + tighten suffix regex [9bc52d62]
* feat(multiqc): DC manage flow + folder uploads + live cache + prewarm [46233d62]
* Add sample metadata TSV generation to multi-run report script [b68228c5]
* Add multi-run MultiQC parquet report generator [6a4a929c]
* ci(docker): pin pnpm@10 in viewer-builder + dev-viewer stages [57d098d4]
* helm: roll Pods on ConfigMap/Secret change via include-based checksum [fdab0bfc]
* Update CHANGELOG.md for v0.11.2 [541835e3]
* Bump version: 0.11.1 → 0.11.2 [5359b861]
* viewer: declare react-resizable as direct dep [6e9a4d50]
* viewer: fix Docker build for prod + dev compose HMR [0f9cb772]
* Update CHANGELOG.md for v0.11.1 [363f05e8]
* Bump version: 0.11.0 → 0.11.1 [c7c2e32e]
* Move viralrecon recipes to canonical projects/ path [909a55ac]
* Update CHANGELOG.md for v0.11.0 [2539176f]
* Bump version: 0.10.3 → 0.11.0 [d4eb9e37]
...
```plaintext
ghcr.io/depictio/depictio:1.0.1-b2
ghcr.io/depictio/depictio:latest
ghcr.io/depictio/depictio:stable
ghcr.io/depictio/depictio:edge
```
...
<details>
<summary>Click to expand the changelog for 1.0.1-b2</summary>
...
* Bump version: 1.0.1-b1 → 1.0.1-b2 [abf41d96]
* fix(quickstart): let mongo start under cap_drop:[ALL] [24e64ba0]
* Update CHANGELOG.md for v1.0.1-b1 [dbb9536a]
* Bump version: 1.0.0 → 1.0.1-b1 [7721b7f1]
* fix(quickstart): keep working creds in .env.example for CI/dev compose [850876c8]
...
```plaintext
ghcr.io/depictio/depictio:1.0.1-b1
ghcr.io/depictio/depictio:latest
ghcr.io/depictio/depictio:stable
ghcr.io/depictio/depictio:edge
```
...
<details>
<summary>Click to expand the changelog for 1.0.1-b1</summary>
...
* Bump version: 1.0.0 → 1.0.1-b1 [512c35ca]
* fix(quickstart): keep working creds in .env.example for CI/dev compose [850876c8]
* ci(quickstart): also build arm64 for beta releases [372eafd8]
* ci(quickstart): build arm64 only for stable releases [a4a79525]
* ci(quickstart): verify admin user + iris dashboard seeded in smoke test [aed39f8f]
...
<details>
<summary>Click to expand the changelog for 1.0.0</summary>
...
* Bump version: 1.0.0-b9 → 1.0.0 [7cd21243]
* Update CHANGELOG.md for v1.0.0-b9 [89b63dc4]
* Bump version: 1.0.0-b8 → 1.0.0-b9 [0ee3c1b2]
* chore(screenshots): regenerate reference dashboard thumbnails [b801f220]
* fix(multiqc): drop matplotlib dep from general-stats colormap [d15ce24a]
...
```plaintext
ghcr.io/depictio/depictio:1.0.0-b9
ghcr.io/depictio/depictio:latest
ghcr.io/depictio/depictio:stable
ghcr.io/depictio/depictio:edge
```
...
<details>
<summary>Click to expand the changelog for 1.0.0-b9</summary>
...
* Bump version: 1.0.0-b8 → 1.0.0-b9 [3d72bba0]
* chore(screenshots): regenerate reference dashboard thumbnails [b801f220]
* fix(multiqc): drop matplotlib dep from general-stats colormap [d15ce24a]
* Update CHANGELOG.md for v1.0.0-b8 [5933db29]
* Bump version: 1.0.0-b7 → 1.0.0-b8 [03c2e0ab]
...
```plaintext
ghcr.io/depictio/depictio:1.0.0-b8
ghcr.io/depictio/depictio:latest
ghcr.io/depictio/depictio:stable
ghcr.io/depictio/depictio:edge
```
...
<details>
<summary>Click to expand the changelog for 1.0.0-b8</summary>
...
* Bump version: 1.0.0-b7 → 1.0.0-b8 [766e7082]
* test(catalog): invoke moved maintainer commands via the dev app [651ef000]
* feat(cli): add `depictio commands` grouped reference table [900caaab]
* refactor(cli): move the data link subgroup under the hidden dev group [8e4ec380]
* refactor(cli): move catalog match/compose to the hidden dev group [eb56367a]
...
```plaintext
ghcr.io/depictio/depictio:1.0.0-b7
ghcr.io/depictio/depictio:latest
ghcr.io/depictio/depictio:stable
ghcr.io/depictio/depictio:edge
```
...
<details>
<summary>Click to expand the changelog for 1.0.0-b7</summary>
...
* Bump version: 1.0.0-b6 → 1.0.0-b7 [785cf1ac]
* fix(viewer): tab menu refresh + icon handling in tab/dashboard modals ([#795](https://github.com/depictio/depictio/pull/795)) [d40615ac]
* Remove producers.py suggestion engine + migrate ampliseq complex_heatmap tiles to use: ([#797](https://github.com/depictio/depictio/pull/797)) [edcbdb84]
* fix(screenshots): install sans-serif fonts in worker image + settle before capture ([#794](https://github.com/depictio/depictio/pull/794)) [203b6fcb]
* feat(helm): migrate MongoDB HA from Community Operator to Percona PSMDB Operator ([#792](https://github.com/depictio/depictio/pull/792)) [1f67a28b]
...
```plaintext
ghcr.io/depictio/depictio:1.0.0-b6
ghcr.io/depictio/depictio:latest
ghcr.io/depictio/depictio:stable
ghcr.io/depictio/depictio:edge
```
...
<details>
<summary>Click to expand the changelog for 1.0.0-b6</summary>
...
* Bump version: 1.0.0-b5 → 1.0.0-b6 [c450346d]
* fix(auth): multi-replica key-wipe race breaking dev-demo ingestion + React default-thumbnail fallback ([#791](https://github.com/depictio/depictio/pull/791)) [f6c8015e]
* Bump version: 1.0.0-b4 → 1.0.0-b5 [34b2df74]
* Helm chart: viewer startup order, MongoDB Community Operator HA, resource rebalance ([#788](https://github.com/depictio/depictio/pull/788)) [3f706bae]
* Bump version: 1.0.0-b3 → 1.0.0-b4 [673f98df]
...
<details>
<summary>Click to expand the changelog for 1.0.0-b2</summary>
...
* Bump version: 1.0.0-b1 → 1.0.0-b2 [24bc6ed5]
* fix(settings): relax password constraints for usability ([#786](https://github.com/depictio/depictio/pull/786)) [d19fbef7]
* Remove -beta suffix from React routes (Dash→React cutover) ([#785](https://github.com/depictio/depictio/pull/785)) [f25f03be]
* Update CHANGELOG.md for v1.0.0-b1 [4d616ed1]
* fix(bump): correct SKIP hook ID after dash removal [0bab4634]
...
* Bump version: 0.13.11 → 0.13.12 [856ac173]
* fix(filters): cross-DC link filter uses join column, not filter column ([#777](https://github.com/depictio/depictio/pull/777)) [0ac305dd]
* Update CHANGELOG.md for v0.13.11 [1537c445]
* Bump version: 0.13.10 → 0.13.11 [55f3cbb9]
* Update CHANGELOG.md for v0.13.10 [2ebf6988]
...
* Bump version: 0.13.10 → 0.13.11 [25b826f5]
* Update CHANGELOG.md for v0.13.10 [2ebf6988]
* Bump version: 0.13.9 → 0.13.10 [a43e1e0a]
* fix(builder): viz suggestion precision, text-step skip, screenshot regen on Save ([#775](https://github.com/depictio/depictio/pull/775)) [592f9188]
* Update CHANGELOG.md for v0.13.9 [315b7079]
...
* Bump version: 0.13.9 → 0.13.10 [0744cc71]
* fix(builder): viz suggestion precision, text-step skip, screenshot regen on Save ([#775](https://github.com/depictio/depictio/pull/775)) [592f9188]
* Update CHANGELOG.md for v0.13.9 [315b7079]
* Bump version: 0.13.8 → 0.13.9 [9e7e4695]
* fix(auth): let admins bypass public/demo-mode write-action gates ([#774](https://github.com/depictio/depictio/pull/774)) [c11a794f]
...
* Bump version: 0.13.8 → 0.13.9 [0afc0ad4]
* fix(auth): let admins bypass public/demo-mode write-action gates ([#774](https://github.com/depictio/depictio/pull/774)) [c11a794f]
* Update CHANGELOG.md for v0.13.8 [02415436]
* Bump version: 0.13.7 → 0.13.8 [b24c7bd8]
* Fix React project import 404 and per-project creation timestamps ([#773](https://github.com/depictio/depictio/pull/773)) [5bbe89a4]
...
* Bump version: 0.13.7 → 0.13.8 [ad72e282]
* Fix React project import 404 and per-project creation timestamps ([#773](https://github.com/depictio/depictio/pull/773)) [5bbe89a4]
* fix(screenshots): honour dashboard-level owners after duplication ([#771](https://github.com/depictio/depictio/pull/771)) [6fd87c6b]
* Update CHANGELOG.md for v0.13.7 [7bd55279]
* Bump version: 0.13.6 → 0.13.7 [49962549]
...
* Bump version: 0.13.6 → 0.13.7 [8359e903]
* fix(deltatables): use positional indexing in precompute_columns_specs ([#769](https://github.com/depictio/depictio/pull/769)) [c5f7af62]
* Update CHANGELOG.md for v0.13.6 [172f24fd]
* Bump version: 0.13.5 → 0.13.6 [4b492162]
* fix(seeds): force format=tsv on recipe DC conversion ([#768](https://github.com/depictio/depictio/pull/768)) [fccb1004]
...
* Bump version: 0.13.5 → 0.13.6 [67e139fb]
* fix(seeds): force format=tsv on recipe DC conversion ([#768](https://github.com/depictio/depictio/pull/768)) [fccb1004]
* Update CHANGELOG.md for v0.13.5 [1e2e29b8]
* Bump version: 0.13.4 → 0.13.5 [aee046ea]
* feat(viralrecon): bundle 15 canonical seed TSVs + missing recipes + CI check ([#767](https://github.com/depictio/depictio/pull/767)) [9400824c]
...
* Bump version: 0.13.4 → 0.13.5 [746d5096]
* feat(viralrecon): bundle 15 canonical seed TSVs + missing recipes + CI check ([#767](https://github.com/depictio/depictio/pull/767)) [9400824c]
* Update CHANGELOG.md for v0.13.4 [f8b852a0]
* Bump version: 0.13.3 → 0.13.4 [c00a1d2a]
* feat(viralrecon): bundle minimal scan-target test data into image ([#766](https://github.com/depictio/depictio/pull/766)) [e293e51f]
...
* Bump version: 0.13.3 → 0.13.4 [7f3aa5a3]
* feat(viralrecon): bundle minimal scan-target test data into image ([#766](https://github.com/depictio/depictio/pull/766)) [e293e51f]
* Update CHANGELOG.md for v0.13.3 [4cb1b2ac]
* Bump version: 0.13.2 → 0.13.3 [eba8199b]
* test(seeds): assert all reference-dashboard dc_ids match STATIC_IDS [98f63f4a]
...
* Bump version: 0.13.1 → 0.13.2 [95b1e0c9]
* fix(viewer): enable dashboard pinning in public/demo mode [a12461f5]
* fix(ampliseq): add metadata→multiqc_data DCLink to 2.16.0 template [7429ddb1]
* fix(cli): process materialised recipe DCs as file scans [61d23f1d]
* fix(seeds): preserve resolved DATA_ROOT for viralrecon [089606d2]
...
* Bump version: 0.13.0 → 0.13.1 [2f30295d]
* feat(card-builder): live preview for top_n / coverage / concentration [78a31a88]
* fix(card-builder): Mantine 7 group format for Multi-metric select [58370a66]
* fix(seeds): preserve source=transformed on recipe DCs after materialisation [b6220e2b]
* fix(card): persist multi-metric config + guard against missing outliers [90324f44]
...
* Bump version: 0.12.2 → 0.13.0 [9dbc764f]
* ci(ampliseq): download phylogenetic tree + taxonomy + ASV table [f6dcdc03]
* fix(ci): align iris dashboard title with seed after polish commit [ff571c4e]
* test(deltatables): mock collect_schema for filter-applied path [4d87e311]
* merge: resolve conflicts with main [d29572d4]
...
* Bump version: 0.12.1 → 0.12.2 [af0ed42c]
* feat(viewer): gate figure Code Mode behind ownership + deployment mode [1f15d7cb]
* fix(viewer): cast Plotly selection-clear payloads for strict types [1c44f785]
* fix(viewer): add @types/plotly.js so CI tsc finds plotly types [ae674ec7]
* fix(viewer): declare 'plotly.js' module so tsc accepts the import [e2bd8056]
...
* Bump version: 0.12.0 → 0.12.1 [35b1806c]
* fix(screenshots): restore correct dark variant for 646b0f3c1e4a2d7f8e5b8ca2 [4a2e0c99]
* chore(screenshots): refresh dashboard thumbnails [1f200cc0]
* feat(docs): playwright-driven docs screenshot capture for React viewer [8930f8a5]
* feat(viewer): NotesFooter read-only for non-owners [039baa95]
...
* Bump version: 0.11.2 → 0.12.0 [43bde510]
* feat(viewer): inline coordinates detection in Create DC modal [a73373c7]
* feat(viewer): dashboards + projects revamp (toolbar, table, pins, filters) [fdcd3eed]
* fix(viewer-dev): unbreak favicon under Vite's base prefix [62c077fe]
* fix(dev): unblock side-by-side worktree dev stacks [03129fe7]
...
* Bump version: 0.11.1 → 0.11.2 [9fb6e079]
* viewer: declare react-resizable as direct dep [6e9a4d50]
* viewer: fix Docker build for prod + dev compose HMR [0f9cb772]
* Update CHANGELOG.md for v0.11.1 [363f05e8]
* Bump version: 0.11.0 → 0.11.1 [c7c2e32e]
...
* Bump version: 0.11.0 → 0.11.1 [ea965691]
* Update CHANGELOG.md for v0.11.0 [2539176f]
* Bump version: 0.10.3 → 0.11.0 [d4eb9e37]
* viewer: gate write actions in public/demo mode (UI + API) [6f1645e8]
* viewer: unify per-mode button gating, drop anonymous public flow, fix resize handles [49f19fcd]
...
* Bump version: 0.10.1 → 0.10.2 [dc01fe56]
* feat(deployment): add annotations for config checksum in backend and frontend [59768b20]
* Update CHANGELOG.md for v0.10.1 [ca160d30]
* Bump version: 0.10.0 → 0.10.1 [b72b35a8]
* fix(test): set glob_pattern=None when bypassing RecipeSource validation [398e7dea]
...
er for test data generation [818803a8]
...
* feat: Add project template system with  [49ab01f8]

#### Bug Fixes 🐛

* fix: remove unused inheritDefaultAnnotations — dead code after annotation refactor [9876e82a]
* fix: always render base nginx annotations on dedicated backend/minio ingresses [81215227]
* fix: correct test assertion for template.yaml preference in locate_template [020d4a56]
...
* Bump version: 0.9.0 → 0.10.0 [ccef7ba7]
* revert: remove Serve-specific permission/auth logic from generic chart template [a87bfe10]
* Fix cross-DC link filtering + heatmap annotations on filter [503a4d01]
* Fix metadata links: source_column sample→ID (matches actual metadata column name) [0de966f6]
* Fix heatmap annotations disappearing on filter + add metadata→heatmap link [b19a625e]
...
* Graceful handling of missing data collections [7b434454]
* Remove alpha diversity/rarefaction from base dashboard [cf633c1a]
* Add missing taxonomy_heatmap.py recipe to git [2d0d1c15]
* Fix taxonomy_composition: detect taxonomy by column name, not cell value [5dbdcac6]
* CI: fix CLI package build by using uv as installer [c824b69b]
* Generic metadata flow: replace hardcoded habitat/sampling_date with template variables [ca43ae6d]
* Sync ampliseq 2.16.0 dashboard components with 0.8.2/main version [5ceec417]
* Fix TSV files read as comma-separated: auto-add separator=tab for tsv format [f30d637c]
* Fix ampliseq 2.16.0: init all DCs, metadata column, base dashboard [5627546d]
* CI: derive samplesheet from Metadata_full.tsv sample IDs [2988ab3e]
* Fix template init: apply conditionals and skip unresolved-var DCs [300c5664]
* Add optional field to DataCollection model to support template DC hints [d609717c]
* Strip template-only DC fields (optional) before Pydantic model validation [60624757]
* Fix ruff linting issues in test files [3715b50a]
* Fix ruff formatting in test files [ba15fc73]
* Replace pipeline-specific integration tests with generic parametrized recipe validation [5975e588]
* Trim ampliseq integration tests to discovery-only (no per-recipe transform tests) [2a34ca5f]
* Implement optional METADATA_FILE template support with generic agnostic tests [fad07665]
* Exclude Jupyter notebooks from GitHub language statistics [3dca507a]
* Format deltatables.py [dff90113]
* Exclude dev/, packages/, scripts/ from ruff linting [d3b33956]
* Move recipes into projects/ with versioned-then-shared fallback [fbd59d3b]
* Simplify template/recipe code: reuse, quality and efficiency fixes [b514955d]
* Remove expected_files/expected_directories from template system [1d5cb14f]
* Simplify template preflight checks: remove --deep and redundant expected_files [e4c6d3da]
* Add automatic dashboard import to template system [306b97c5]
* Install multiqc optional dep in CLI CI environment [e6f874f3]
* Fix recipe run: typer.Exit(0) caught by except Exception [cf1d8d09]
* Replace ampliseq CI verification with proper execution [2ea9fae9]
* Fix test_layout_auto_generation for split-panel layout [b16a377a]
* Fix test: layout data now uses split-panel fields [ab01687a]
* Fix test assertion: CLI outputs 'Schema/domain validation failed' [7ef57f55]
* Fix test_interactive_component_validates to match model behavior [c23031fe]
* Fix ruff formatting in settings_models.py [c05e68fa]
* Add depictio.recipes to CLI package for test discovery [a9c87a05]
* Fix CI: comment out ty check, fix type errors in our files [503db051]
* Fix test failures: remove heavy beanie import chain, add tests/unit to testpaths [8be796e0]
* Fix pre-commit issues: import sorting and notebook output stripping [cb52fa00]
* Add template & recipe unit tests and enable quality checks in CI [0f090b6e]
* Use absolute() instead of resolve() for data_root to preserve symlinks [3d6450e7]
* Add --preview-recipes flag to show recipe input/output without writing [8ac4a61a]
* Fix ModuleNotFoundError for load_recipe in recipe dc_ref resolution [08323770]
* Resolve dc_ref sources in recipe execution for cross-DC dependencies [d1f44a0f]
* Replace direct MongoDB call in CLI with API endpoint for dc_specific_properties [a3316dab]
* Add fallback import for depictio.recipes from source tree [64f8e863]
* Filter stale file records in fetch_file_data before validation [0704a086]
* Fix stale file cleanup when re-running with different template/data_root [771aaf74]
* Fix setuptools config to include all depictio sub-packages [0563f9bd]
* Add bundled pre-computed data files for ampliseq 2.16.0 init [ddf13021]
* Move default init project from ampliseq 2.14.0 to 2.16.0 [da8681ae]
* Fix download script default path to use relative directory [f17d5124]
* Add ampliseq 2.16.0 template and template origin badges in UI [696933ef]
* Wire up data_dir resolution in process_recipe_data_collection [96ccbdc2]
* Fix remaining recipe paths and strip notebook outputs [996274f3]
* Fix recipe paths, remove ancom_volcano, use ANCOM-BC for everything [e1eae796]
* Update CHANGELOG.md for v0.9.0 [2142bfa6]
* Bump version: 0.8.2 → 0.9.0 [a6bba6c2]
...
ectivity test from workflow [6ee1d30]
